...
|---|---|
| 状態 | 採用（設計確定）・未実装 |
| 決定日 | 2026-04-24 頃（2026-05-09 トリガー条件拡張・調査確定、2026-05-10 メッシュサイズ改訂、2026-05-18 フェーズ配置をフェーズ3.5 縮小版パイプラインのループ構造に確定・N=6 までエスカレーション化、2026-06-02 FR-014 を単発実行に簡略化・エスカレーション責務を FR-022 コル充足判定に移管） |
...
- **基本方針: 縮小版パイプラインのループ**（ループはコル充足判定 FR-022 が制御、FR-014 は単発実行）:
...
5. **merged 系ファイルの更新方法**: FR-014 自身は merged.csv を**直接書き換えない**。
   ...
   FR-014 自身の責務は「広域モード呼び出し → 広域 per-mesh CSV 出力」のみ（単発実行）。
   FR-008 の再実行・エスカレーション判定（N=4→5→6）・フェーズ4 移行判断はコル充足判定（FR-022）が担う。

6. **実装原則: per-mesh エンジン（C++）の拡張・再利用（ロジック重複禁止）**
   FR-014 は per-mesh 解析エンジン（[ADR-SRS-010](ADR-SRS-010-cpp-opencv-migration.md) 移行後の `findsummits` C++ 実装）を拡張して対応し、
   ...
   - 広域 per-mesh CSV の出力（FR-022 コル充足判定へ返却）
   のみとし、標高解析ロジックは C++ エンジン側に留める。
   対象ピーク特定・FR-008 再実行・エスカレーション判定（N=4→5→6）・フェーズ4 移行判断は FR-022 コル充足判定が担う。
