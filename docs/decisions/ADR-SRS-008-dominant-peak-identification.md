...
データ問題による包含なしを想定してフォールバックを設けた。しかし [FR-014](../20_SRS.md#fr-014-広域結合解析オーケストレーション) のフェーズ3.5 広域再解析（N=4→5→6 エスカレーション）完了後に `key_col_resolved=false` が残るのは陸地最高峰（島嶼部最高峰含む）に限られ、発生頻度が極めて稀。稀少ケースを機械的に処理するより、ログ警告 + 処理中止で人手判断に委ねるほうが安全。→ フォールバック廃止（2026-05-19）
