...
`delete_zone_max_drop` の値は **200m** とする（2026-05-21 実測にて確定）。

確定値の算出:
```
delete_zone_max_drop = 150 + ceil(max_abs_diff / 50) * 50
                     = 150 + ceil(17.95 / 50) * 50
                     = 150 + 50 = 200
```

- 150m: SOTA プロミネンス閾値
- 17.95m: JA サミット 833 件（タイルキャッシュ済み）の abs(SOTA登録標高 − DEM標高) 最大値
- 50m 単位切り上げ: 余裕込みで切りのよい整数

実測の詳細: `analysis/sota_dem_elevation_diff.py` を 4,600 件の JA サミット全件に対して実行（2026-05-21）。タイルキャッシュ済みの 833 件で有効値を取得。abs_diff の平均 2.33m・中央値 1.35m・最大 17.95m。
...
- **独立峰問題の自然解消**: 200m キャップによりポリゴンが日本全土規模に膨張しない
...
旧方針を維持し「既存 SOTA プロミネンス > 500m なら削除除外」をガード条件として追加する案。500m 値が恣意的で、独立峰問題への根本対処にならない。delete判定ゾーン方式では 200m キャップにより独立峰問題が自然解消するため、ガード条件自体が不要になる。
