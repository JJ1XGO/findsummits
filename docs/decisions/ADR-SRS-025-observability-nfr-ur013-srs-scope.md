...
URD に追加されたが、SRS 側に受け皿が無く UR-013 → SRS のトレースが切れていた。
この欠落が FR-018 のループ内再入設計を何度も揺れさせた原因である（[ADR-SRS-024](ADR-SRS-024-fr018-loop-reentry-and-peak-filter.md)）。

UR-013 が要求する内容は性質の異なる2つの要求に分解できる:
...
UR-013 を以下の方針で SRS に反映する:

**(a) 観測可能性（①）は NFR-009 として受ける**

NFR-009「観測可能性（中間成果物の可視化）」を新設（`対応 UR: UR-013`）し、
FR-016/FR-018 がその実現手段であることをトレースする。
...
**1. UR-013 を FR として受ける案（却下）**

「中間成果物を可視化できること」を新規 FR（例: FR-023）として起こす案。
...
- UR-013 → NFR-009 → FR-016/FR-018 のトレースが SRS 内で閉じる
- FR-018 のループ内再入設計（[ADR-SRS-024](ADR-SRS-024-fr018-loop-reentry-and-peak-filter.md)）の根拠が URD/SRS 両レベルで確立する
- NFR-009 の保証範囲外（コル座標の地図表示）は [ADR-SRS-026](ADR-SRS-026-intermediate-geojson-peak-col-visualization.md) として明示し、スコープの曖昧さを解消する
