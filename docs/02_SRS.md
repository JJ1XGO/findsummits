...
この SRS の機能要件（FR-XXX）でパラメータを参照するときは、本セクションで定義した名称を使用すること。外部I/F は [6. 外部インターフェース仕様](#6-外部インターフェース仕様)、ユーザ入力は [7.1 ユーザ入力一覧](#71-ユーザ入力一覧)、内部データは [7.2 内部データ一覧](#72-内部データ一覧) で定義した名称を使用すること。定義の重複を防ぎ、変更時の更新箇所を一元化するためのセクションである（[ADR-SRS-016](decisions/ADR-SRS-016-data-classification-external-user-internal.md) 参照）。
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 標高タイル（キャッシュ） | 内部データ | PNG（DEM 種別・ズームレベル・タイル座標ごとに1ファイル） | キャッシュ保存先は [7.2 参照](#72-内部データ一覧) |
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| N03 行政区域 ZIP | ユーザ入力 | 必須 | — | [7.1.3 参照](#713-n03-行政区域-zip) |
...
| 標高タイル（キャッシュ） | `$DATA_DIR/tiles/{z}/{x}/{y}_{suffix}.png`<br>`{suffix}`: `a`=DEM5a / `b`=DEM5b・DEM10b / `c`=DEM5c | [FR-001](#fr-001-標高タイル事前取得) | [FR-002](#fr-002-dem-階層フォールバック) / [FR-004](#fr-004-メッシュ結合解析) | |
