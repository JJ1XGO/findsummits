...
4. [機能要件](#4-機能要件)
   ...
   - [フェーズ2-1: 3x3メッシュ解析](#フェーズ2-1-3x3メッシュ解析)
   ...
   - [フェーズ2-2: 広域メッシュ解析](#フェーズ2-2-広域メッシュ解析)
     - [FR-014: 独立峰のコル探索](#fr-014-独立峰のコル探索)
   - [フェーズ3: ピーク統合・コル充足判定](#フェーズ3-ピーク統合コル充足判定)
...
| ピーク候補リスト | Union-Find 構造を含むピーク候補集合（ピーク座標・標高・山塊グループ情報） | [FR-005](#fr-005-ピーク候補検出) → [FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)/[FR-016](#fr-016-ピーク域ポリゴン生成) |
| コル情報 | 各ピークのコル座標・コル標高・プロミネンス・`key_col_resolved` フラグ | [FR-006](#fr-006-コル検出プロミネンス計算) → [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)/[FR-016](#fr-016-ピーク域ポリゴン生成) |
| per-mesh ピーク候補 CSV | フェーズ2（[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)）が出力する 1次メッシュ単位のピーク候補リスト（通常モード）。カラム定義・ファイル仕様は [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) 出力仕様参照 | [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) → [FR-008](#fr-008-per-mesh-csv-統合) |
| 広域 per-mesh ピーク候補 CSV | [FR-004](#fr-004-メッシュ結合解析) N≥4 モード が出力するピーク候補 CSV（[FR-014](#fr-014-独立峰のコル探索) の実現方式参照）。通常 per-mesh と区別できる命名（具体規則は HLD） | [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) → [FR-008](#fr-008-per-mesh-csv-統合) |
...
- **概要**: メッシュコードリストの各メッシュを N×N 単位（中心+隣接最大 N²-1）で解析対象として結合画像を生成し、per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)〜[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)・[FR-016](#fr-016-ピーク域ポリゴン生成)）に引き渡すオーケストレーション。
...
**処理**:
  ...
  - **N≥4 モード（フェーズ2-2 広域解析）**: [FR-022](#fr-022-コル充足判定) から「対象ピーク座標リスト」と N（4・5・6 のいずれか）を受け取り、以下の追加動作を行う:
    ...
    - **対象ピーク絞り込み出力**: 解析範囲内で検出された全ピークではなく、対象ピーク座標リストに含まれるピーク行のみを per-mesh CSV に出力する（[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) 出力動作参照）
    - **ポリゴン生成のスキップ**: [FR-016](#fr-016-ピーク域ポリゴン生成) は呼び出さない（広域モードの目的はコル特定のみで、ポリゴンは通常 per-mesh の結果を使用する）
    - **ファイル命名**: 通常 per-mesh と区別できる命名で出力する（詳細は [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) 出力動作・HLD 参照）
...
| データ名 | 形式 | 備考 |
|---|---|---|
| ピーク候補リスト | メモリ上。[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぎ | Union-Find 構造を含む |
...
コル充足判定（[FR-022](#fr-022-コル充足判定)）から `key_col_resolved=false` ピークの広域解析を要求された場合のみ実行する。N×N + L14 max pooling の広域モード（[FR-004](#fr-004-メッシュ結合解析) の N≥4 モード）で per-mesh パイプラインを実行し、対象ピークのコル確定を試みる。実現方式の概要は [FR-014](#fr-014-独立峰のコル探索) を参照。
...
- 後段の [FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) は通常 3×3 モードと同じパイプラインで処理する（[FR-016](#fr-016-ピーク域ポリゴン生成) は呼び出さない）
...
- 出力（[FR-004](#fr-004-メッシュ結合解析) → [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) 経由）: 広域 per-mesh ピーク候補 CSV
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| per-mesh ピーク候補 CSV | 必須 | — | 通常モード・広域混在可。ファイル仕様は [FR-007 出力参照](#fr-007-プロミネンスフィルタper-mesh-csv-出力) |
...
- **詳細**:
  ...
  - プロミネンス最終フィルタ: **プロミネンス最終フィルタ閾値**（[データ辞書参照](#221-設定可能項目)）以上（[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) の一次フィルタ通過済みのレコードに適用）
