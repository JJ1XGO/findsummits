...
- 無効値のピクセルは山頂検出・プロミネンス計算の対象から除外する
...
- 解析対象メッシュ全体の四隅の緯度経度を求め、その緯度経度をもとに標高タイル（ズームレベル 15）のタイル番号を特定する（計算式は HLD 参照）
...
- Union-Find（連結成分管理アルゴリズム）でピークをグループ管理する
...
- **対応 UR**: [UR-001](01_URD.md#ur-001)
- 実測値（9メッシュ最大解析時）: 解析ピーク時 90.8%（≒56.9GB）、物理メモリ 62.72GB + スワップ 39.7GB で動作確認済み
...
- **対応 UR**: [UR-009](01_URD.md#ur-009)
- 解析エンジンはメッシュごとに `$DATA_DIR/logs/<meshcode>.log` を出力する
- stdout とログファイルに同時出力する（二重出力）
- 記録内容: 解析開始・解析範囲・タイル読み込み進捗・標高地形図 PNG 出力・解析処理時間・CSV 保存件数・解析完了・エラー発生時の失敗理由
