...
6. [外部インターフェース仕様](#6-外部インターフェース仕様)
   - [6.1 入出力: 地理院標高タイル（取得・ローカルキャッシュ）](#61-入出力-地理院標高タイル取得ローカルキャッシュ)
   ...
   - [6.14 入力: コマンド引数（1次メッシュコード）](#614-入力-コマンド引数1次メッシュコード)
   - [6.15 入力: N03 行政区域データ（生）](#615-入力-n03-行政区域データ生)
   - [6.16 入力: HTML ビューア上のユーザー入力](#616-入力-html-ビューア上のユーザー入力)
   - [6.17 永続化: localStorage 編集内容](#617-永続化-localstorage-編集内容)
...
この SRS の機能要件（FR-XXX）でパラメータ・FR 間中間データを参照するときは、本セクションで定義した名称を使用すること。外部インターフェースを参照するときは [6. 外部インターフェース仕様](#6-外部インターフェース仕様) で定義した名称を使用すること。定義の重複を防ぎ、変更時の更新箇所を一元化するためのセクションである（[ADR-SRS-014](decisions/ADR-SRS-014-srs-parameter-description-convention.md) 参照）。
...
FR の入出力テーブル「データ名」セルは以下で定義した名称を使用する（[ADR-SRS-014 規約 5](decisions/ADR-SRS-014-srs-parameter-description-convention.md) 参照）。

FR 間で受け渡されるデータ（メモリ上または一時ファイル）。

| データ名 | 意味 | 生成 FR → 消費 FR |
|---|---|---|
| DEM フォールバック適用後の標高値 | DEM5a/5b/5c/DEM10b の優先順フォールバックを適用したピクセル標高値（m）。-9999m は無効値センチネル | [FR-002](#fr-002-dem-階層フォールバック) → [FR-003](#fr-003-標高デコードnodata-処理) |
| デコード済みピクセル標高値 | RGB エンコードから変換し、NODATA・海面下を統一処理した標高値（m） | [FR-003](#fr-003-標高デコードnodata-処理) → [FR-004](#fr-004-メッシュ結合解析)/[FR-005](#fr-005-ピーク候補検出) |
| 結合画像 | N×N メッシュ分のタイルを結合したメモリ上の標高ピクセル群（+1px 海面ボーダー付き） | [FR-004](#fr-004-メッシュ結合解析) → [FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-016](#fr-016-ピーク域ポリゴン生成) |
| ピーク候補リスト | Union-Find 構造を含むピーク候補集合（ピーク座標・標高・山塊グループ情報） | [FR-005](#fr-005-ピーク候補検出) → [FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) |
| コル情報 | 各ピークのコル座標・コル標高・プロミネンス・`key_col_resolved` フラグ | [FR-006](#fr-006-コル検出プロミネンス計算) → [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) |
| per-mesh ピーク候補 CSV | フェーズ2（[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）が出力する 1次メッシュ単位のピーク候補リスト（通常モード）。カラム定義・ファイル仕様は [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) 出力仕様参照 | [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) → [FR-008](#fr-008-per-mesh-csv-統合) |
| 広域 per-mesh ピーク候補 CSV | [FR-004](#fr-004-メッシュ結合解析) N≥4 モード が出力するピーク候補 CSV（[FR-014](#fr-014-独立峰のコル探索) の実現方式参照）。通常 per-mesh と区別できる命名（具体規則は HLD） | [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) → [FR-008](#fr-008-per-mesh-csv-統合) |
| per-mesh アクティベーションゾーン GeoJSON | フェーズ2（[FR-016](#fr-016-ピーク域ポリゴン生成)）が出力する 1次メッシュ単位のピーク域ポリゴン（AZ + delete判定ゾーン）。ファイル仕様は [FR-016](#fr-016-ピーク域ポリゴン生成) 出力仕様参照 | [FR-016](#fr-016-ピーク域ポリゴン生成) → [FR-018](#fr-018-per-mesh-activationgeojson-統合) |
| 統合ピーク候補 work CSV（`merged_peak.csv`） | per-mesh CSV を統合・重複排除した内部 work CSV。[FR-009](#fr-009-sotaリスト突合match_status-判定) の入力として使用 | [FR-008](#fr-008-per-mesh-csv-統合) → [FR-009](#fr-009-sotaリスト突合match_status-判定)/[FR-022](#fr-022-コル充足判定) |
| 統合済みピーク域 GeoJSON（`merged_activation.geojson`） | per-mesh アクティベーションゾーン GeoJSON を統合した内部中間ファイル。デバッグ・差分検査用として物理出力を残す。ファイル仕様は [FR-018](#fr-018-per-mesh-activationgeojson-統合) 出力仕様参照 | [FR-018](#fr-018-per-mesh-activationgeojson-統合) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| N03 前処理済み地域 GeoJSON（都道府県/振興局単位） | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する都道府県/振興局単位の行政区域 GeoJSON。SOTA エリアコード自動付与に使用。詳細は [6.10](#610-入力-n03-前処理済みファイルfr-017-生成) 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| N03 前処理済み市区町村 GeoJSON | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する市区町村単位の行政区域 GeoJSON。所在地（市区町村名）取得に使用。詳細は [6.10](#610-入力-n03-前処理済みファイルfr-017-生成) 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| 北方領土除外タイルリスト | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する北方領土に該当するタイル座標リスト（ズームレベル15、`x y` 形式、1行1タイル）。詳細は [6.10](#610-入力-n03-前処理済みファイルfr-017-生成) 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-003](#fr-003-標高デコードnodata-処理) |
...
| データ名 | 形式 | 備考 |
|---|---|---|
| 地理院標高タイル | PNG（DEM 種別・ズームレベル・タイル座標ごとに1ファイル） | キャッシュ保存先は [6.1 参照](#61-入出力-地理院標高タイル取得ローカルキャッシュ) |
...
|---|---|---|---|
| N03 行政区域データ | 必須 | — | [6.15 参照](#615-入力-n03-行政区域データ生) |
| N03 行政区域データ年版 | 必須 | — | [データ辞書参照](#221-設定可能項目) |
...
|---|---|---|---|
| 1次メッシュコードリスト | 必須 | — | — |
| 解析範囲サイズ N | 任意 | 3。フェーズ2-2（[FR-022](#fr-022-コル充足判定)）から呼び出された場合は N=4〜6 が渡される | [データ辞書参照](#221-設定可能項目) |
| 対象ピーク座標リスト | 任意 | —（N=3 通常モード時は不要） | N≥4 モード（フェーズ2-2）時に必須。[FR-022](#fr-022-コル充足判定) が抽出した `key_col_resolved=false` ピーク群（緯度経度）。後述の N≥4 モード処理で 4 コーナーパターン生成と対象ピーク行絞り込み出力に使用 |

**出力**:

| データ名 | 形式 | 備考 |
|---|---|---|
| 結合画像 | メモリ上 | 解析対象メッシュ範囲情報を伴う。後段の FR-005/006/016/007 に引き渡す |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 地理院標高タイル | 必須 | — | FR-004 のタイル読み込みからピクセル単位で渡される |

**出力**:

| データ名 | 形式 |
|---|---|
| DEM フォールバック適用後の標高値 | 浮動小数（m）。有効値または -9999m（無効値センチネル） |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 結合画像 | 必須 | — | FR-004 のタイル読み込み完了直後・解析処理開始前に渡される |
...
| データ名 | 形式 | 備考 |
|---|---|---|
| ピーク候補リスト | メモリ上。[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぎ | Union-Find 構造を含む |
...
| データ名 | 形式 |
|---|---|
| コル情報 | コル座標・コル標高・プロミネンス・`key_col_resolved` フラグ |
...
| データ名 | 形式 |
|---|---|
| per-mesh ピーク候補 CSV | CSV。ファイル: `$DATA_DIR/results/csv/<meshcode>.csv`（`<meshcode>` は4桁の1次メッシュコード）、エンコーディング: UTF-8 |
...
| データ名 | 形式 |
|---|---|
| per-mesh アクティベーションゾーン GeoJSON | GeoJSON（2種類のポリゴンを同一ファイルに収録）。ファイル: `$DATA_DIR/results/csv/<meshcode>_activation.geojson`、形式: GeoJSON（RFC 7946）、座標参照系: WGS84（EPSG:4326）。プロパティ仕様は下記「詳細」参照 |
- **詳細**:
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| per-mesh ピーク候補 CSV | 必須 | — | 通常モード・広域混在可。ファイル仕様は [FR-007 出力参照](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) |
...
| データ名 | 形式 |
|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`） | CSV（内部 work CSV。[FR-009](#fr-009-sotaリスト突合match_status-判定) の入力として使用される中間ファイル） |
- **詳細**:
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| per-mesh アクティベーションゾーン GeoJSON | 必須 | — | 通常 per-mesh のみ（広域モード = [FR-004](#fr-004-メッシュ結合解析) N≥4 モードは GeoJSON を生成しない） |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`） | 必須 | — | [FR-008](#fr-008-per-mesh-csv-統合) 出力 |

**出力**:

| データ名 | 形式 | 備考 |
|---|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`） | CSV | フェーズ2-2 ループ実行後の更新版（[FR-008](#fr-008-per-mesh-csv-統合) 再実行を経由） |
| フェーズ4 移行可否 | 判定結果 | 全ピーク `key_col_resolved=true` なら pass。N=6 を使い切っても未解決が残る場合はそのままフェーズ4 へ移行（[FR-009](#fr-009-sotaリスト突合match_status-判定) の `is_key_col_unresolved` 不備フラグで異常終了する） |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`） | 必須 | — | 全ピーク `key_col_resolved=true`・全ポリゴン `area_complete=true` が前提。不備は不備フラグで後続に引き継ぐ |
| 統合済みピーク域 GeoJSON（`merged_activation.geojson`） | 必須 | — | 通常 per-mesh の GeoJSON のみから統合 |
| SOTA サミットリスト CSV | 必須 | — | JA プレフィックスサミット一覧 |
| 1次メッシュコードリスト | 任意 | 未指定時=全範囲対象。[FR-010](#fr-010-削除候補のスコープ) の削除候補スコープ判定に使用 | — |
| SOTA 既存サミット GeoJSON | 必須 | — | 日本語山岳名取得用（[6.11 参照](#611-入力-sota-既存サミット-geojsongeojson_vn)） |

**出力**:

| データ名 | 形式 | 備考 |
|---|---|---|
| 統合ピーク候補（`merged.geojson`） | GeoJSON | フェーズ4 末尾の中心成果物。全フィーチャ・rationale・不備フラグ metadata を含む |
| サミット一覧（突合後）（`merged_summit.xlsx`） | XLSX | バッチ生成時点の確認用。HTML ビューア編集内容は反映しない |
- **詳細**:
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 1次メッシュコードリスト | 任意 | 未指定時は全 SOTA サミットを削除候補対象とする（全国フル解析を前提） | FR-009 に渡されたものと同じリスト |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 統合ピーク候補（`merged.geojson`） | 必須 | — | FR-009 出力の中心成果物 |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 統合ピーク候補（`merged.geojson`） | 必須 | — | 作業用 HTML ビューアに埋め込み済み |
| localStorage 編集内容 | 必須 | — | [FR-019](#fr-019-html-ビューア機能仕様) が管理 |
...
| データ名 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|
| 統合ピーク候補（`merged.geojson`） | 必須 | — | 作業用 HTML ビューアに埋め込み済み（[FR-013](#fr-013-html-ビューア生成) 出力） |
...
|---|---|---|---|
| 統合ピーク候補（`merged.geojson`） | 必須 | — | 作業用 HTML ビューアに埋め込み済み |
| HTML ビューア上のユーザー入力 | 必須 | — | 山岳名・rationale 編集値。[FR-019](#fr-019-html-ビューア機能仕様) が管理 |
...
| 入出力の役割 | [FR-001](#fr-001-標高タイル事前取得) がリモートから取得してキャッシュに書き込む（出力）。[FR-002](#fr-002-dem-階層フォールバック)/[FR-004](#fr-004-メッシュ結合解析) がキャッシュから読み込む（入力） |
...
### 6.16 入力: HTML ビューア上のユーザー入力

| 項目 | 仕様 |
|---|---|
| 用途 | 申請書・サミット一覧（申請内容反映版）に転記する山岳名と、申請根拠（rationale）の編集 |
| 入力フィールド | 山岳名（日本語）・山岳名（英語）・rationale（各ピークの申請根拠テキスト） |
| 管理 FR | [FR-019](#fr-019-html-ビューア機能仕様) が入力フィールドを制御し、[6.17](#617-永続化-localstorage-編集内容) に永続化する |

### 6.17 永続化: localStorage 編集内容

| 項目 | 仕様 |
|---|---|
| 用途 | HTML ビューア（[6.16](#616-入力-html-ビューア上のユーザー入力)）上の入力・編集内容をブラウザセッション間で保持する |
