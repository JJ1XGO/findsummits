...
| SOTA 既存サミット GeoJSON | 突合に使用する既存 SOTA サミット GeoJSON ファイル群（`ref/geojson_v{N}/`）。バージョン番号 N は 2.2.1 設定可能項目「SOTA 既存サミット GeoJSON バージョン」で指定 | GeoJSON（`ja0.geojson`〜`ja9.geojson`）。フォーマットは 6.11 参照 |
| N03 行政区域データ | 国土数値情報の市区町村単位 GeoJSON。年版は 2.2.1 設定可能項目「N03 行政区域データ年版」で指定 | GeoJSON（市区町村単位の行政区域ポリゴン）。出典: [ref/SOURCES.md](../ref/SOURCES.md) |
| HTML ビューア上のユーザー入力 | HTML ビューアでユーザーが入力する山岳名（日本語・英語）および rationale 編集値 | テキスト。[FR-019](#fr-019-html-ビューア機能仕様) が管理 |
| localStorage 編集内容 | HTML ビューア上の入力・編集内容のブラウザ内永続化データ | JSON 形式。[FR-019](#fr-019-html-ビューア機能仕様) が localStorage に保存・管理 |
...
| per-mesh ピーク候補 CSV | フェーズ2（[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)）が出力する 1次メッシュ単位のピーク候補リスト（通常モード）。フォーマットは [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) 出力仕様参照 | [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) → [FR-008](#fr-008-per-mesh-csv-統合) |
...
| per-mesh アクティベーションゾーン GeoJSON | フェーズ2（[FR-016](#fr-016-ピーク域ポリゴン生成)）が出力する 1次メッシュ単位のピーク域ポリゴン（AZ + delete判定ゾーン）。GeoJSON（Polygon フィーチャ集合） | [FR-016](#fr-016-ピーク域ポリゴン生成) → [FR-018](#fr-018-per-mesh-activationgeojson-統合) |
...
| 統合済みピーク域 GeoJSON（`merged_activation.geojson`） | per-mesh アクティベーションゾーン GeoJSON を統合した内部中間ファイル。デバッグ・差分検査用として物理出力を残す | [FR-018](#fr-018-per-mesh-activationgeojson-統合) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| N03 前処理済み地域 GeoJSON（都道府県/振興局単位） | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する都道府県/振興局単位の行政区域 GeoJSON。SOTA エリアコード自動付与に使用。フォーマットは 6.10 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| N03 前処理済み市区町村 GeoJSON | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する市区町村単位の行政区域 GeoJSON。所在地（市区町村名）取得に使用。フォーマットは 6.10 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| 北方領土除外タイルリスト | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する北方領土に該当するタイル座標リスト（ズームレベル15、`x y` 形式、1行1タイル） | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-003](#fr-003-標高デコードnodata-処理) |
...
| データ名 | 意味 | 形式・備考 |
|---|---|---|
| 統合ピーク候補（`merged.geojson`） | **中心成果物**。フェーズ3〜4 を通じて構築される全国統合 GeoJSON。全 Point・Polygon・LineString・rationale・不備フラグ metadata を含む（[ADR-SRS-013](decisions/ADR-SRS-013-merged-geojson-as-central-data.md)） | GeoJSON FeatureCollection。フォーマットは 6.6 参照 |
| 作業用 HTML ビューア（`merged_viewer.html`） | [FR-013](#fr-013-html-ビューア生成) が生成する編集可能なローカル HTML ビューア。`merged.geojson` データを JavaScript 変数として埋め込み | 静的 HTML |
| 申請書 XLSX | SOTA 日本支部への山岳リスト更新申請書。[FR-011](#fr-011-申請書-xlsx-生成) が HTML ビューア内でブラウザ生成する | XLSX 形式。フォーマットは 6.3 参照 |
| サミット一覧（突合後）（`merged_summit.xlsx`） | [FR-009](#fr-009-sotaリスト突合match_status-判定) がバッチ生成するサミット一覧。HTML ビューアでの編集内容は反映しない | XLSX 形式。フォーマットは 6.5 参照 |
| サミット一覧（申請内容反映版）（`merged_summit_revised.xlsx`） | [FR-012](#fr-012-サミット一覧申請内容反映版生成) が HTML ビューア内でブラウザ生成するサミット一覧。HTML ビューアでの編集内容を反映 | XLSX 形式 |
| 標高地形図 PNG | [FR-015](#fr-015-標高地形図出力) が生成する解析範囲の標高色分け PNG。目視確認用 | PNG。長辺 6000px に縮小 |
| 公開用 HTML ファイル | [FR-020](#fr-020-公開用-html-ビューア生成) がブラウザ生成する閲覧専用の自己完結型 HTML。GitHub Pages 等での外部公開を想定 | HTML。フォーマットは 6.13 参照 |
| 申請エビデンス ZIP | [FR-021](#fr-021-申請エビデンス-zip-生成) がブラウザ生成する申請エビデンス一式 ZIP（XLSX + 4 GeoJSON） | ZIP |
...
|---|---|---|
| N03 行政区域データ（全国・市区町村単位 GeoJSON。ユーザーが事前にダウンロードして配置する。[データ辞書参照](#222-入出力データ)） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| per-mesh ピーク候補 CSV（通常モード・広域混在可。[データ辞書参照](#222-入出力データ)） | 必須 | — |
...
| データ名 | 形式 |
|---|---|
| 統合済みピーク域 GeoJSON（merged_activation.geojson） | GeoJSON（**内部中間ファイル**。[FR-009](#fr-009-sotaリスト突合match_status-判定) の point-in-polygon 突合に使用。デバッグ・差分検査用として物理出力は残す。詳細: [ADR-SRS-013](decisions/ADR-SRS-013-merged-geojson-as-central-data.md)） |
- **詳細**:
...
| SOTA 既存サミット GeoJSON（[データ辞書参照](#222-入出力データ)。日本語山岳名取得用） | 必須 | — |
