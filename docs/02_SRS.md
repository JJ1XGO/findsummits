...
FR の入出力テーブル「データ名」セルは以下で定義した名称を使用する（[ADR-SRS-014 規約 5](decisions/ADR-SRS-014-srs-parameter-description-convention.md) 参照）。派生形補語（リスト・ピクセル等）は FR 側で括弧書きで補ってよい。
...
| 統合ピーク候補 work CSV（`merged_peak.csv`） | per-mesh CSV を統合・重複排除した内部 work CSV。[FR-009](#fr-009-sotaリスト突合match_status-判定) の入力として使用 | [FR-008](#fr-008-per-mesh-csv-統合)/[FR-014](#fr-014-独立峰のコル探索) → [FR-009](#fr-009-sotaリスト突合match_status-判定)/[FR-014](#fr-014-独立峰のコル探索) |
...
|---|---|---|
| N03 行政区域データ（全国・市区町村単位 GeoJSON。ユーザーが事前にダウンロードして配置する。[6.15 参照](#615-入力-n03-行政区域データ生)） | 必須 | — |
| N03 行政区域データ年版（[データ辞書参照](#221-設定可能項目)） | 必須 | — |
...
| データ名 | 形式 |
|---|---|
| 標高タイル（ローカルキャッシュ） | PNG（DEM 種別・ズームレベル・タイル座標ごとに1ファイル） |
...
|---|---|---|
| 1次メッシュコード（リスト） | 必須 | — |
| 解析範囲サイズ N（[データ辞書参照](#221-設定可能項目)） | 任意 | 3。[FR-014](#fr-014-独立峰のコル探索) からの呼び出し時は N=4/5/6 が指定される |

**出力**:

| データ名 | 形式 |
|---|---|
| 結合画像（+解析対象メッシュ範囲。後段の [FR-005](#fr-005-ピーク候補検出)/006/016/007 に引き渡し、最終的に per-mesh ピーク候補 CSV・per-mesh アクティベーションゾーン GeoJSON として出力） | メモリ上 |
...
**処理**:
  ...
  - N≥4 の場合（[FR-014](#fr-014-独立峰のコル探索) 呼び出し時）は [FR-016](#fr-016-ピーク域ポリゴン生成) のポリゴン生成を呼び出さない。また [FR-014](#fr-014-独立峰のコル探索) から呼び出された場合は対象ピークの行だけを per-mesh CSV に出力する（詳細は [FR-014](#fr-014-独立峰のコル探索) 参照）
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 標高タイル（[FR-004](#fr-004-メッシュ結合解析) のタイル読み込みから渡されるピクセル） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| DEM フォールバック適用後の標高値（[FR-002](#fr-002-dem-階層フォールバック) のフォールバック適用後） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 結合画像（[FR-004](#fr-004-メッシュ結合解析) のタイル読み込み完了直後、解析処理開始前） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 結合画像（[FR-004](#fr-004-メッシュ結合解析) から渡される） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| ピーク候補リスト（Union-Find 構造を含む） | メモリ上。[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぎ |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ピーク候補リスト（[FR-005](#fr-005-ピーク候補検出) から渡される） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ピーク候補リスト・コル情報（[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算) から） | 必須 | — |
| 結合画像（[FR-004](#fr-004-メッシュ結合解析) から） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| コル情報（[FR-006](#fr-006-コル検出プロミネンス計算) から） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| per-mesh ピーク候補 CSV（通常モード・広域混在可。ファイル仕様は [FR-007 出力参照](#fr-007-プロミネンスフィルタper-mesh-csv-出力)） | 必須 | — |
| 1次メッシュコード（リスト） | 任意 | 未指定時は利用可能な全 per-mesh CSV を読み込む |
...
- **詳細**:
  ...
  - **再入可能性**: 本機能は [FR-014](#fr-014-独立峰のコル探索) のループから複数回呼び出され、その都度 merged_peak.csv（内部 work CSV）が再生成される
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| per-mesh アクティベーションゾーン GeoJSON（[FR-016](#fr-016-ピーク域ポリゴン生成) 出力。通常 per-mesh のみ） | 必須 | — |
| 1次メッシュコード（リスト） | 任意 | 未指定時は利用可能な全 per-mesh GeoJSON を読み込む |
...
- **詳細**:
  ...
  - **再入可能性**: [FR-014](#fr-014-独立峰のコル探索) のループ中は再入しない。広域モードは GeoJSON を生成しないため、本機能は [FR-014](#fr-014-独立峰のコル探索) 以前の1回のみ実行される
...
- **概要**: merged_peak.csv の `key_col_resolved=false` ピークに対して、N×N メッシュ + L14 max pooling の広域モードで [FR-004](#fr-004-メッシュ結合解析) ピーク解析を再呼び出しし、Key コルを特定する。フェーズ3（統合の直後・フェーズ4 突合の直前）に位置する。
**入力**:

| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`）（[FR-008](#fr-008-per-mesh-csv-統合) 出力） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 広域 per-mesh ピーク候補 CSV（通常 per-mesh と区別できるファイル名。命名規則の詳細は HLD 参照） | CSV |
| 統合ピーク候補 work CSV（`merged_peak.csv`）（[FR-008](#fr-008-per-mesh-csv-統合) 再実行後） | CSV |
- **詳細**:
  ...
  - **基本フロー（縮小版パイプラインのループ）**:
    ...
    2. **エスカレーション・ループ（N = 4, 5, 6 の順）**:
       ...
       - **[FR-004](#fr-004-メッシュ結合解析) を「処理モード = N×N + L14」パラメータ付きで呼び出す**。per-mesh パイプライン（[FR-004](#fr-004-メッシュ結合解析)→[FR-005](#fr-005-ピーク候補検出)→[FR-006](#fr-006-コル検出プロミネンス計算)→[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)）が広域モードで実行され、広域 per-mesh CSV が出力される（[FR-016](#fr-016-ピーク域ポリゴン生成) のポリゴン生成は実行しない。広域再解析の目的は Key コル特定のみであり、ポリゴンは通常 per-mesh の結果を使用する）
  ...
  - **実装設計**（詳細は [`decisions/ADR-SRS-004`](decisions/ADR-SRS-004-level14-max-pooling-isolated-peaks.md) Consequences 参照）:
    ...
    - 1px ボーダー: 現行実装を踏襲（L14 での 1px 幅増加は [FR-014](#fr-014-独立峰のコル探索) 対象ピークへの影響なし、許容）
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`）（[FR-008](#fr-008-per-mesh-csv-統合)/[FR-014](#fr-014-独立峰のコル探索) 出力。全ピークが `key_col_resolved=true` かつ全ポリゴンが `area_complete=true` であることを前提とする。不備は不備フラグとして後続に引き継ぐ） | 必須 | — |
| 統合済みピーク域 GeoJSON（`merged_activation.geojson`）（[FR-018](#fr-018-per-mesh-activationgeojson-統合) 出力。通常 per-mesh の GeoJSON のみから統合） | 必須 | — |
| SOTA サミットリスト CSV（JA プレフィックスサミット一覧） | 必須 | — |
| 1次メッシュコード（リスト） | 任意 | 未指定時=全範囲対象。[FR-010](#fr-010-削除候補のスコープ) の削除候補スコープ判定に使用 |
| SOTA 既存サミット GeoJSON（[6.11 参照](#611-入力-sota-既存サミット-geojsongeojson_vn)。日本語山岳名取得用） | 必須 | — |
...
| サミット一覧（突合後）（`merged_summit.xlsx`）（バッチ生成時点の確認用。カラム構成は [FR-012](#fr-012-サミット一覧申請内容反映版生成) と同じ。HTML ビューアでの編集内容は反映しない） | XLSX |
- **詳細**:
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 1次メッシュコード（リスト）（[FR-009](#fr-009-sotaリスト突合match_status-判定) に渡されたもの） | 任意 | 未指定時は全 SOTA サミットを削除候補対象とする（全国フル解析を前提） |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補（merged.geojson）（[FR-009](#fr-009-sotaリスト突合match_status-判定) 出力の中心成果物） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補（`merged.geojson`）（作業用 HTML ビューアに埋め込み。[FR-013](#fr-013-html-ビューア生成) 出力） | 必須 | — |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補（`merged.geojson`）（作業用 HTML ビューアに埋め込み） | 必須 | — |
...
| 使用 FR | [FR-001](#fr-001-標高タイル事前取得)・[FR-004](#fr-004-メッシュ結合解析) が受け取る |
