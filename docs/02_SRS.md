...
|---|---|---|---|
| タイル取得コンポーネント | DEM タイルを国土地理院から取得・ローカルキャッシュ | メッシュコード、取得設定 | ローカルキャッシュ済み PNG タイル |
...
- **概要**: 国土数値情報の市区町村単位の行政区域 GeoJSON（約 12 万ポリゴン）から、後段 FR が利用する 3 種類の派生データを生成する初回限定のデータ準備処理。なぜ前処理が必要か:
  ...
  - 北方領土（[ADR-URD-005](decisions/ADR-URD-005-northern-territories-exclusion.md) 参照）はタイル読み込み段階（[FR-004](#fr-004-メッシュ結合解析)）で除外したいが、C エンジンに GIS ライブラリを持ち込まずに済むよう、除外対象タイルのリストを事前生成する
...
| 北方領土除外タイルリスト | 内部データ | テキスト | [FR-004](#fr-004-メッシュ結合解析) での NODATA マスクに使用。詳細仕様は [7.2.1](#721-n03-前処理済みファイル詳細仕様) 参照 |
...
- **生成 3: 北方領土除外タイルリスト**:
  ...
  - **このリストの使われ方**: FR-017 はリストを生成するだけ。実際の除外は [FR-004](#fr-004-メッシュ結合解析)（C エンジン側）がリストを読み込み、タイル読み込み時に該当タイルの全ピクセルを NODATA (R=128, G=0, B=0) として扱うことで実現する
- **フォールバック（後段 FR の動作）**:
  ...
  - 北方領土除外タイルリストが未生成の場合: [FR-004](#fr-004-メッシュ結合解析) は北方領土タイルの NODATA 化をスキップする
...
**説明**:
  ...
  - **北方領土タイル除外**: タイル読み込み時に [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成する北方領土除外タイルリストを参照し、リスト内のタイル（ズームレベル 15 の x/y 座標）は全ピクセルを NODATA (R=128, G=0, B=0) として扱う。リスト未存在時の動作は [FR-017](#fr-017-n03-行政区域前処理データ準備) を参照（詳細: [`decisions/ADR-URD-005-northern-territories-exclusion.md`](decisions/ADR-URD-005-northern-territories-exclusion.md)）
...
| 北方領土除外タイルリスト | — | [FR-017](#fr-017-n03-行政区域前処理データ準備) | [FR-004](#fr-004-メッシュ結合解析) | 詳細仕様は [7.2.1](#721-n03-前処理済みファイル詳細仕様) 参照 |
...
| 項目 | 仕様 |
|---|---|
| 用途 | 北方領土に該当するタイルを解析対象から除外するためのリスト。タイル読み込み時に参照し、リスト内のタイルは全ピクセルを NODATA として扱う（[FR-004](#fr-004-メッシュ結合解析)） |
...
- 一部の 1 次メッシュには北方領土が含まれるが、北方領土に所在するピークは SOTA 日本支部の管轄外のため解析対象外とする。除外方法は [FR-004](#fr-004-メッシュ結合解析)・[FR-017](#fr-017-n03-行政区域前処理データ準備) で規定する（タイル単位除外。根拠: [ADR-URD-005](decisions/ADR-URD-005-northern-territories-exclusion.md)）
