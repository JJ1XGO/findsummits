...
| per-mesh ピーク候補 CSV | フェーズ2（FR-007）が出力する 1次メッシュ単位のピーク候補リスト（通常モード）。フォーマットは FR-007 出力仕様参照 | [FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) → [FR-008](#fr-008-per-mesh-csv-統合) |
| 広域 per-mesh ピーク候補 CSV | FR-014 が広域モードで出力するピーク候補 CSV。通常 per-mesh と区別できる命名（具体規則は HLD） | [FR-014](#fr-014-独立峰のコル探索) → [FR-008](#fr-008-per-mesh-csv-統合) |
| per-mesh アクティベーションゾーン GeoJSON | フェーズ2（FR-016）が出力する 1次メッシュ単位のピーク域ポリゴン（AZ + delete判定ゾーン）。GeoJSON（Polygon フィーチャ集合） | [FR-016](#fr-016-ピーク域ポリゴン生成) → [FR-018](#fr-018-per-mesh-activationgeojson-統合) |
| 統合ピーク候補 work CSV（`merged_peak.csv`） | per-mesh CSV を統合・重複排除した内部 work CSV。FR-009 の入力として使用 | [FR-008](#fr-008-per-mesh-csv-統合)/[FR-014](#fr-014-独立峰のコル探索) → [FR-009](#fr-009-sotaリスト突合match_status-判定)/[FR-014](#fr-014-独立峰のコル探索) |
...
| N03 前処理済み地域 GeoJSON（都道府県/振興局単位） | FR-017 が生成する都道府県/振興局単位の行政区域 GeoJSON。SOTA エリアコード自動付与に使用。フォーマットは 6.10 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| N03 前処理済み市区町村 GeoJSON | FR-017 が生成する市区町村単位の行政区域 GeoJSON。所在地（市区町村名）取得に使用。フォーマットは 6.10 参照 | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-009](#fr-009-sotaリスト突合match_status-判定) |
| 北方領土除外タイルリスト | FR-017 が生成する北方領土に該当するタイル座標リスト（ズームレベル15、`x y` 形式、1行1タイル） | [FR-017](#fr-017-n03-行政区域前処理データ準備) → [FR-003](#fr-003-標高デコードnodata-処理) |
...
| 作業用 HTML ビューア（`merged_viewer.html`） | FR-013 が生成する編集可能なローカル HTML ビューア。`merged.geojson` データを JavaScript 変数として埋め込み | 静的 HTML |
| 申請書 XLSX | SOTA 日本支部への山岳リスト更新申請書。FR-011 が HTML ビューア内でブラウザ生成する | XLSX 形式。フォーマットは 6.3 参照 |
| サミット一覧（突合後）（`merged_summit.xlsx`） | FR-009 がバッチ生成するサミット一覧。HTML ビューアでの編集内容は反映しない | XLSX 形式。フォーマットは 6.5 参照 |
| サミット一覧（申請内容反映版）（`merged_summit_revised.xlsx`） | FR-012 が HTML ビューア内でブラウザ生成するサミット一覧。HTML ビューアでの編集内容を反映 | XLSX 形式 |
| 標高地形図 PNG | FR-015 が生成する解析範囲の標高色分け PNG。目視確認用 | PNG。長辺 6000px に縮小 |
| 公開用 HTML ファイル | FR-020 がブラウザ生成する閲覧専用の自己完結型 HTML。GitHub Pages 等での外部公開を想定 | HTML。フォーマットは 6.13 参照 |
| 申請エビデンス ZIP | FR-021 がブラウザ生成する申請エビデンス一式 ZIP（XLSX + 4 GeoJSON） | ZIP |
...
|---|---|---|
| 1次メッシュコード（リスト） | 必須 | — |
| 解析範囲サイズ N（[データ辞書参照](#221-設定可能項目)） | 任意 | 3。FR-014 からの呼び出し時は N=4/5/6 が指定される |

**出力**:

| データ名 | 形式 |
|---|---|
| 結合画像（+解析対象メッシュ範囲。後段の FR-005/006/016/007 に引き渡し、最終的に per-mesh ピーク候補 CSV・per-mesh アクティベーションゾーン GeoJSON として出力） | メモリ上 |
...
| データ名 | 形式 |
|---|---|
| 統合ピーク候補 work CSV（`merged_peak.csv`） | CSV（内部 work CSV。FR-009 の入力として使用される中間ファイル） |
- **詳細**:
  ...
  - プロミネンス最終フィルタ: **プロミネンス最終フィルタ閾値**（[データ辞書参照](#221-設定可能項目)）以上（FR-007 の一次フィルタ通過済みのレコードに適用）
...
| 1次メッシュコード（リスト） | 任意 | 未指定時=全範囲対象。FR-010 の削除候補スコープ判定に使用 |
