...
この SRS の機能要件（FR-XXX）でパラメータ・入出力データを参照するときは、本セクションで定義した名称を使用すること。定義の重複を防ぎ、変更時の更新箇所を一元化するためのセクションである（[ADR-SRS-014](decisions/ADR-SRS-014-srs-parameter-description-convention.md) 参照）。
