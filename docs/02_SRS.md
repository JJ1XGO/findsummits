...
- **概要**: 国土数値情報の市区町村単位の行政区域 GeoJSON（約 12 万ポリゴン）から、後段 FR が利用する 3 種類の派生データを生成する初回限定のデータ準備処理。なぜ前処理が必要か:
  ...
  - 北方領土（[ADR-URD-005](decisions/ADR-URD-005-northern-territories-exclusion.md) 参照）はピーク検出段階（[FR-003](#fr-003-標高デコードnodata-処理)）で除外したいが、C エンジンに GIS ライブラリを持ち込まずに済むよう、除外対象タイルのリストを事前生成する
...
| 北方領土除外タイルリスト | 内部データ | テキスト | [FR-003](#fr-003-標高デコードnodata-処理) での NODATA マスクに使用。詳細仕様は [7.2.1](#721-n03-前処理済みファイル詳細仕様) 参照 |
...
- **生成 3: 北方領土除外タイルリスト**:
  - N03_007（行政区域コード）が 01696〜01701 に一致するポリゴン（択捉・国後・色丹の北方領土 6 村。詳細: [ADR-URD-005](decisions/ADR-URD-005-northern-territories-exclusion.md)）を抽出
  ...
  - **このリストの使われ方**: FR-017 はリストを生成するだけ。実際の除外は [FR-003](#fr-003-標高デコードnodata-処理)（C エンジン側）がリストを読み込み、該当タイルの全ピクセルを NODATA(-9999m) として扱うことで実現する
- **フォールバック（後段 FR の動作）**:
  ...
  - 北方領土除外タイルリストが未生成の場合: [FR-003](#fr-003-標高デコードnodata-処理) は北方領土タイルの NODATA 化をスキップする
...
| 北方領土除外タイルリスト | — | [FR-017](#fr-017-n03-行政区域前処理データ準備) | [FR-003](#fr-003-標高デコードnodata-処理) | 詳細仕様は [7.2.1](#721-n03-前処理済みファイル詳細仕様) 参照 |
...
| 生成方法 | [FR-017](#fr-017-n03-行政区域前処理データ準備) で生成（北方領土6村に相当する行政区域コード（N03_007: 01696〜01701）のポリゴン内タイルを列挙） |
| 省略時の動作 | 詳細は [FR-017 参照](#fr-017-n03-行政区域前処理データ準備) |
