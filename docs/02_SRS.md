...
|---|---|---|
| N03 行政区域データ（全国・市区町村単位 GeoJSON。ユーザーが事前にダウンロードして配置する） | 必須 | — |
| N03 行政区域データ年版（[データ辞書参照](#341-設定可能項目)） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| N03 前処理済み地域 GeoJSON（都道府県/振興局単位） | GeoJSON（47 都道府県 + 北海道 14 振興局 = 計 61 地域）。[FR-009](#fr-009-sotaリスト突合match_status-判定) での SOTA エリアコード自動付与に使用 |
| N03 前処理済み市区町村 GeoJSON | GeoJSON（約 2,000 地域）。[FR-009](#fr-009-sotaリスト突合match_status-判定) での所在地（市区町村名）取得に使用 |
| 北方領土除外タイルリスト | テキスト（x y 形式、1 行 1 タイル、ズームレベル 15）。[FR-003](#fr-003-標高デコードnodata-処理) での NODATA マスクに使用 |
...
| データ名 | 形式 |
|---|---|
| ローカルキャッシュ済み標高タイル | PNG（DEM 種別・ズームレベル・タイル座標ごとに1ファイル） |
...
**処理**:
  ...
  - タイル取得の HTTP リクエストに **User-Agent 識別子**（[データ辞書参照](#341-設定可能項目)）を付与する（地理院サーバー側でアクセス元を特定・問い合わせできるようにするため）
  - **タイル取得のリクエスト間隔**（[データ辞書参照](#341-設定可能項目)）を守ってリクエストする
  - **タイル取得の最大並列数**（[データ辞書参照](#341-設定可能項目)）で並行取得する
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 標高タイルのピクセル（[FR-004](#fr-004-メッシュ結合解析) のタイル読み込み処理から渡される） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| フォールバック適用後の標高値 | 浮動小数（m）。有効値または -9999m（無効値センチネル） |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| PNG タイルピクセル（[FR-002](#fr-002-dem-階層フォールバック) のフォールバック適用後） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| デコード済みピクセル標高値 | 浮動小数（m）。NODATA は -9999m、海面下は ELEV_SEA=0.0m に統一 |
...
- **詳細**:
  ...
  - **北方領土タイル除外**: タイル読み込み時に FR-017 が生成する北方領土除外タイルリストを参照し、リスト内のタイル（ズームレベル15の x/y 座標）は全ピクセルを無効値（-9999m）として扱う。リスト未存在時の動作は [FR-017 参照](#fr-017-n03-行政区域前処理データ準備)（詳細: [`decisions/ADR-URD-005-northern-territories-exclusion.md`](decisions/ADR-URD-005-northern-territories-exclusion.md)）
  - **竹島の除外**: 竹島が含まれるメッシュ 5531 を解析対象メッシュコードリストから除外することで対応する。N03 ポリゴンベースのタイルマスク（北方領土と同方式）は使用しない。FR-003 での追加処理はない（詳細: [`decisions/ADR-URD-009-takeshima-exclusion.md`](decisions/ADR-URD-009-takeshima-exclusion.md)）
...
- **概要**: メッシュコードリストの各メッシュを N×N 単位（中心+隣接最大 N²-1）で解析対象として結合画像を生成し、per-mesh パイプライン（FR-005〜FR-007・FR-016）に引き渡すオーケストレーション。
...
|---|---|---|
| 1次メッシュコードリスト | 必須 | — |
| 解析範囲サイズ N（[データ辞書参照](#341-設定可能項目)） | 任意 | 3。FR-014 からの呼び出し時は N=4/5/6 が指定される |

**出力**:

| データ名 | 形式 |
|---|---|
| 結合画像 + 解析対象メッシュ範囲 | メモリ上（後段の FR-005/006/016/007 に引き渡し、最終的に per-mesh ピーク候補 CSV・per-mesh アクティベーションゾーン GeoJSON として出力） |
...
**処理**:
  ...
  - N≥4 の場合（FR-014 呼び出し時）は [FR-016](#fr-016-ピーク域ポリゴン生成) のポリゴン生成を呼び出さない。また FR-014 から呼び出された場合は対象ピークの行だけを per-mesh CSV に出力する（詳細は [FR-014](#fr-014-独立峰のコル探索) 参照）
  - 解析・ピーク検出・コル検出のロジック自体は N の値によらず共通（[FR-005](#fr-005-ピーク候補検出)・[FR-006](#fr-006-コル検出プロミネンス計算)・[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)）
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 結合画像のピクセル群（[FR-004](#fr-004-メッシュ結合解析) から渡される） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| ピーク候補リスト + 山塊グループ | Union-Find 構造（[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力) に引き継ぎ） |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ピーク候補と山塊グループ（[FR-005](#fr-005-ピーク候補検出) から渡される） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 各ピークのコル情報 | コル座標・コル標高・プロミネンス・`key_col_resolved` フラグ |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ピーク座標・プロミネンス・コル標高（FR-005/006 から） | 必須 | — |
| 結合画像（FR-004 から） | 必須 | — |
...
| データ名 | 形式 |
|---|---|
| per-mesh アクティベーションゾーン GeoJSON | GeoJSON（2種類のポリゴンを同一ファイルに収録） |
- **詳細**:
  ...
  - **delete判定ゾーンポリゴン**（`feature_type="delete_zone"`）:
    - **定義**: 既存 SOTA サミットの削除判定に使用する。ピーク頂上から、プロミネンスと **delete_zone 比高上限**（[データ辞書参照](#341-設定可能項目)）のどちらか小さい方の標高差以内の連続エリア（[ADR-SRS-011](decisions/ADR-SRS-011-delete-zone-polygon.md)）
...
- **概要**: **プロミネンス一次フィルタ閾値**（[データ辞書参照](#341-設定可能項目)）を適用してピーク・コル情報を per-mesh CSV に出力する。

**入力**:

| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ピーク・コル情報（FR-005/006 から） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| per-mesh ピーク候補 CSV | CSV |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 結合画像（FR-004 のタイル読み込み完了直後、解析処理開始前） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 標高地形図 PNG | PNG（人間が視認しやすい配色で標高を色分けしたイメージ。長辺 6000px に縮小） |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| per-mesh ピーク候補 CSV（通常 + 広域混在可） | 必須 | — |
| 1次メッシュコードリスト | 任意 | 未指定時は利用可能な全 per-mesh CSV を読み込む |

**出力**:

| データ名 | 形式 |
|---|---|
| 統合ピーク候補（merged_peak.csv） | CSV（内部 work CSV。FR-009 の入力として使用される中間ファイル） |
- **詳細**:
  ...
  - プロミネンス最終フィルタ: **プロミネンス最終フィルタ閾値**（[データ辞書参照](#341-設定可能項目)）以上（FR-007 の一次フィルタ通過済みのレコードに適用）
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| per-mesh アクティベーションゾーン GeoJSON（FR-016 出力。通常 per-mesh のみ） | 必須 | — |
| 1次メッシュコードリスト | 任意 | 未指定時は利用可能な全 per-mesh GeoJSON を読み込む |

**出力**:

| データ名 | 形式 |
|---|---|
| 統合済みピーク域 GeoJSON（merged_activation.geojson） | GeoJSON（**内部中間ファイル**。FR-009 の point-in-polygon 突合に使用。デバッグ・差分検査用として物理出力は残す。詳細: [ADR-SRS-013](decisions/ADR-SRS-013-merged-geojson-as-central-data.md)） |
- **詳細**:
...
広域モード（[FR-004](#fr-004-メッシュ結合解析) の N×N + L14 パラメータ）で再呼び出しし、
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補（merged_peak.csv）（FR-008 出力） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 広域 per-mesh CSV（通常 per-mesh と区別できるファイル名。命名規則の詳細は HLD 参照） | CSV |
| 更新済み統合ピーク候補（merged_peak.csv）（FR-008 再実行後）| CSV |
- **詳細**:
  ...
  - **基本フロー（縮小版パイプラインのループ）**:
    ...
    2. **エスカレーション・ループ（N = 4, 5, 6 の順）**:
       ...
       - **[FR-004](#fr-004-メッシュ結合解析) を「処理モード = N×N + L14」パラメータ付きで呼び出す**。per-mesh パイプライン（FR-004→[FR-005](#fr-005-ピーク候補検出)→[FR-006](#fr-006-コル検出プロミネンス計算)→[FR-007](#fr-007-プロミネンスフィルタper-mesh-csv-出力)）が広域モードで実行され、広域 per-mesh CSV が出力される（[FR-016](#fr-016-ピーク域ポリゴン生成) のポリゴン生成は実行しない。広域再解析の目的は Key コル特定のみであり、ポリゴンは通常 per-mesh の結果を使用する）
    ...
    3. **最終残存**: N=6 でも `key_col_resolved=false` のピークが残った場合、当該フラグ状態を維持したまま処理を継続する（[FR-009](#fr-009-sotaリスト突合match_status-判定) の `is_key_col_unresolved` 不備フラグが true となり、[FR-009](#fr-009-sotaリスト突合match_status-判定) が異常終了する。[ADR-SRS-011](decisions/ADR-SRS-011-delete-zone-polygon.md)）
  ...
  - **広域 per-mesh ファイル命名**: 通常 per-mesh と区別できるファイル名で出力する。具体的な命名規則の詳細は HLD 参照
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補（merged_peak.csv）（FR-008/FR-014 出力。全ピークが `key_col_resolved=true` かつ全ポリゴンが `area_complete=true` であることを前提とする。不備は不備フラグとして後続に引き継ぐ） | 必須 | — |
| 統合済みピーク域 GeoJSON（merged_activation.geojson）（FR-018 出力。通常 per-mesh の GeoJSON のみから統合） | 必須 | — |
| SOTA サミットリスト CSV（JA プレフィックスサミット一覧） | 必須 | — |
| 1次メッシュコードリスト | 任意 | 未指定時=全範囲対象。FR-010 の削除候補スコープ判定に使用 |
| SOTA 既存サミット GeoJSON（SOTA 既存サミット GeoJSON バージョン（[データ辞書参照](#341-設定可能項目)）に対応するファイル群。日本語山岳名取得用） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 統合ピーク候補（merged.geojson）（フェーズ4 末尾の中心成果物。全 Point/Polygon/LineString・rationale・不備フラグ metadata を含む） | GeoJSON |
| サミット一覧（merged_summit.xlsx）（突合後・バッチ生成時点の確認用。カラム構成は [FR-012](#fr-012-サミット一覧申請内容反映版生成) と同じ。HTML ビューアでの編集内容は反映しない） | XLSX |
- **詳細**:
  ...
  - **市区町村判定**: 各ピーク（matched/new/dominant）および delete サミットの座標を FR-017 が生成した市区町村単位の N03 前処理済み地域 GeoJSON と照合し、`municipality` カラム（例: "根室市"・"標津町"）を merged_summit.xlsx に付与する。市区町村ファイルが存在しない場合は空文字を付与して続行する（警告ログ出力）
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 解析対象メッシュコードリスト（[FR-009](#fr-009-sotaリスト突合match_status-判定) に渡されたもの） | 任意 | 未指定時は全 SOTA サミットを削除候補対象とする（全国フル解析を前提） |

**出力**:

| データ名 | 形式 |
|---|---|
| 削除候補対象サミットの絞り込み結果 | [FR-009](#fr-009-sotaリスト突合match_status-判定) 内のフィルタとして機能 |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| 統合ピーク候補（merged.geojson）（FR-009 出力の中心成果物） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 作業用 HTML ビューア（merged_viewer.html） | 静的 HTML |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ローカル HTML ビューアに埋め込まれた GeoJSON データ（[FR-013](#fr-013-html-ビューア生成) 出力） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| ビューア上の表示・編集状態 | ブラウザ上のインタラクティブ状態 |
| 各エクスポート | [FR-011](#fr-011-申請書-xlsx-生成)・[FR-012](#fr-012-サミット一覧申請内容反映版生成)・[FR-020](#fr-020-公開用-html-ビューア生成)・[FR-021](#fr-021-申請エビデンス-zip-生成) 経由でのブラウザダウンロード |
...
| データ名 | 必須/任意 | デフォルト（任意時） |
|---|---|---|
| ローカル HTML ビューアに埋め込まれた GeoJSON データ | 必須 | — |
| HTML ビューア上のユーザー入力（山岳名・rationale 編集値）（[FR-019](#fr-019-html-ビューア機能仕様) が管理） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| 申請書 XLSX | XLSX（ブラウザダウンロード。テンプレート列 A〜J 構成。フォーマットは [6.3](#63-出力-申請書-xlsx) 参照） |
...
|---|---|---|
| ローカル HTML ビューアに埋め込まれた GeoJSON データ | 必須 | — |
| localStorage の編集内容（[FR-019](#fr-019-html-ビューア機能仕様) が管理） | 必須 | — |

**出力**:

| データ名 | 形式 |
|---|---|
| サミット一覧（申請内容反映版）（`merged_summit_revised.xlsx`） | XLSX（HTML ビューアからブラウザダウンロード。申請エビデンス ZIP 内に同梱） |
...
| データ名 | 形式 |
|---|---|
| 公開用 HTML ファイル | HTML（ブラウザダウンロード。閲覧専用・自己完結型。詳細は [6.13](#613-出力-公開用-html閲覧専用ブラウザダウンロード) 参照） |
...
| データ名 | 形式 |
|---|---|
| 申請エビデンス ZIP | ZIP（ブラウザダウンロード。同梱物は下記「詳細」参照） |
...
- **プロミネンス最終フィルタ閾値**（[データ辞書参照](#341-設定可能項目)）以上のピークを出力すること
- 一次フィルタは**プロミネンス一次フィルタ閾値**（[データ辞書参照](#341-設定可能項目)）（解析範囲境界付近でコルが範囲外に出る場合、プロミネンスが過小評価される可能性があるため、マージンを設けている）
- 最終フィルタ判定は FR-008（フェーズ3）で実施
...
| ファイル | `ref/geojson_v{N}/ja0.geojson` 〜 `ja9.geojson`（{N} は **SOTA 既存サミット GeoJSON バージョン**（[データ辞書参照](#341-設定可能項目)）） |
