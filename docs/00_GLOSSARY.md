...
| 内部トランザクション | FR の入出力として一時的に発生し、メモリ上にのみ存在する値（永続化されず、独立した中央データ構造として保持されない）。SRS セクション 7.2 内部データ一覧の対象外とし、発生する FR の入出力欄に発生元 FR を明示する。例: FR-002 出力「採用 DEM の生 RGB ピクセル値」、FR-003 出力「デコード済みピクセル標高値」（根拠: [ADR-SRS-017](decisions/ADR-SRS-017-internal-transaction-category.md)） |
