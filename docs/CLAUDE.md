...
| ユーザー入力 | ユーザーがツールに給える入力 | コマンド引数・HTML ビューア上のユーザー入力・N03 行政区域 ZIP |
...
- 内部トランザクションは §8.1 の対象外。発生する FR の入出力欄に発生元 FR を明示する（根拠: [ADR-SRS-017](decisions/ADR-SRS-017-internal-transaction-category.md)）
