...
| 作成日 | 2026-04-30 |
| 最終更新日 | 2026-07-11 |
| ステータス | ドラフト（[FR-022](#fr-022-コル充足判定) 純化・[FR-023](#fr-023-解析パイプライン制御) 解析パイプライン制御新設・N 扱い再設計・phase 番号統一。ADR-SRS-027／標高地形図の配色を gist_earth + 陰影起伏方式に確定。ADR-SRS-047） |
...
**説明**:
  ...
  - **異常系**: 該当なし。解析範囲標高グリッド（内部トランザクション）からの決定論的なPNG画像生成であり、失敗しうる外部境界を持たない
...
| 生成タイミング | [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) または [FR-014](#fr-014-広域結合解析オーケストレーション) のオーケストレーション中で解析範囲標高グリッドが生成された直後（[FR-005](#fr-005-ピーク候補検出) 開始前） |
