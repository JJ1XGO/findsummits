...
| 管理 FR | [FR-019](#fr-019-html-ビューア機能仕様) が入力フィールドを制御し、[8.2.2](#822-localstorage-編集内容詳細仕様) に永続化する |
...
| SOTA データベース（`sotadata.org.uk`） | サミットリスト CSV 取得（[6.2.11](#6211-sota-サミットリスト-csv)） |
...
行に FR・NFR、列に UR を配置する。✅ はその FR/NFR が該当 UR の実現に寄与することを示す。UR-009 は削除済みのため列から除外。
