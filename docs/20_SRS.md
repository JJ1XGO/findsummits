...
| 1次メッシュコードリスト | ユーザー入力 | 任意 | 未指定時は利用可能な全 per-mesh CSV を読み込む | — |
...
**説明**:
  ...
  - 同一ピーク座標（ズームレベル15 タイル座標が一致）のレコードを同一ピークとして重複排除し、統合する。広域モードの per-mesh CSV もピーク座標をズームレベル15 座標で出力するため（[FR-014](#fr-014-広域結合解析オーケストレーション) 参照）、本判定は通常・広域の区別なく機能する
    ...
    - `expected_count`: メッシュコードリスト（オプション）をもとに算出する期待解析回数。通常モード行（`analysis_id` 接頭辞 `3`）のみを対象とし、3×3 隣接カウントで算出する。広域モード行（接頭辞 `4/5/6`）は `expected_count` の算出対象外（空欄）。リストが省略された場合も空欄
...
**説明**:
  ...
  - **未充足ループ（N=4→5→6 のエスカレーション）**:
    ...
    3. [FR-008](#fr-008-per-mesh-csv-統合) を再実行して `merged_peak.csv` を再生成する（通常 per-mesh CSV と広域 per-mesh CSV を統合し、`key_col_resolved=true` のレコードがあれば代表として採用する）
...
|---|---|---|---|---|
| per-mesh ピーク候補 CSV | 一時ファイル。ファイル名: `$DATA_DIR/results/csv/<解析識別子>.csv`（通常: `3-<meshcode>.csv`、広域: `<N>-<meshcode>-<コーナー>.csv`） | [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) | [FR-008](#fr-008-per-mesh-csv-統合) | カラム定義は FR-007 出力仕様参照。通常 per-mesh と広域 per-mesh はファイル名のプレフィックスで区別する |
