...
| 9 | 地理院標高タイル | 入力 | PNG（RGB エンコード） | [FR-001](#fr-001-標高タイル事前取得) / [FR-019](#fr-019-html-ビューア機能仕様) | [6.2.9 参照](#629-地理院標高タイル)（FR-001 は事前取得しローカルキャッシュ（内部データ）に保存。FR-019 は等高線オーバーレイ用に実行時取得。FR-004/FR-014 はローカルキャッシュ（内部データ）を参照） |
