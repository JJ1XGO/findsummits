...
7. [ユーザー入力と内部データ](#7-ユーザー入力と内部データ)
   - [7.1 ユーザー入力](#71-ユーザー入力)
     ...
     - [7.1.2 ユーザー入力詳細仕様](#712-ユーザー入力詳細仕様)
       - [7.1.2.1 1次メッシュコードリスト](#7121-1次メッシュコードリスト)
       - [7.1.2.2 HTML ビューア上のユーザー入力](#7122-html-ビューア上のユーザー入力)
       - [7.1.2.3 N03 行政区域 ZIP](#7123-n03-行政区域-zip)
   - [7.2 内部データ](#72-内部データ)
     - [7.2.1 内部データ一覧](#721-内部データ一覧)
     - [7.2.2 内部データ詳細仕様](#722-内部データ詳細仕様)
       - [7.2.2.1 N03 前処理済みファイル詳細仕様](#7221-n03-前処理済みファイル詳細仕様)
       - [7.2.2.2 localStorage 編集内容詳細仕様](#7222-localstorage-編集内容詳細仕様)
8. [外部システム依存関係・環境](#8-外部システム依存関係環境)
9. [制約・前提条件](#9-制約前提条件)
10. [スコープ外](#10-スコープ外)
11. [要求追跡マトリクス](#11-要求追跡マトリクス)
...
この SRS の機能要件（FR-XXX）でパラメータを参照するときは、本セクションで定義した名称を使用すること。外部I/F は [6. 外部インターフェース仕様](#6-外部インターフェース仕様)、ユーザー入力は [7.1 ユーザー入力](#71-ユーザー入力)、内部データは [7.2 内部データ](#72-内部データ) で定義した名称を使用すること。定義の重複を防ぎ、変更時の更新箇所を一元化するためのセクションである（[ADR-SRS-016](decisions/ADR-SRS-016-data-classification-external-user-internal.md) 参照）。
...
|---|---|---|---|---|
| N03 行政区域 ZIP | ユーザー入力 | 必須 | — | [7.1.2.3 参照](#7123-n03-行政区域-zip) |

**出力**:

| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| N03 前処理済み地域 GeoJSON | 内部データ | GeoJSON | 60 地域。[FR-009](#fr-009-sotaリスト突合match_status-判定) での SOTA エリアコード自動付与に使用。詳細仕様は [7.2.2.1](#7221-n03-前処理済みファイル詳細仕様) 参照 |
| N03 前処理済み市区町村 GeoJSON | 内部データ | GeoJSON | 約 2,000 市区町村。[FR-009](#fr-009-sotaリスト突合match_status-判定) での所在地（市区町村名）取得に使用。詳細仕様は [7.2.2.1](#7221-n03-前処理済みファイル詳細仕様) 参照 |
| 北方領土除外タイルリスト | 内部データ | テキスト | [FR-001](#fr-001-標高タイル事前取得) でのタイル取得スキップに使用。詳細仕様は [7.2.2.1](#7221-n03-前処理済みファイル詳細仕様) 参照 |

**説明**:

- **実行タイミング**: 初回のみ。[7.1.2.3](#7123-n03-行政区域-zip) に N03 ZIP が配置され、かつ対応する出力ファイル群が未生成の場合に起動する。N03 ZIP を新年版に差し替えた場合は出力ファイル群を削除してから再実行することで新しい ZIP を反映できる（削除・再実行の操作詳細は HLD 参照）
- **入力 ZIP の検出**: [7.1.2.3](#7123-n03-行政区域-zip) で定義したパターン `N03-(\d{8})_GML\.zip` で走査し、YYYYMMDD が最大のものを採用する。0 件ならエラー終了する。複数候補があった場合は最新を採用し、採用 ZIP 名・YYYYMMDD・サイズをログに記録する
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 標高タイル（ローカルキャッシュ） | 内部データ | PNG | DEM 種別・ズームレベル・タイル座標ごとに 1 ファイル。ローカルキャッシュ保存先は [7.2.1 参照](#721-内部データ一覧) |
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 1次メッシュコードリスト | ユーザー入力 | 任意 | 日本全土1次メッシュコードリスト（全土） | [7.1.2.1 参照](#7121-1次メッシュコードリスト)。指定時は [FR-004](#fr-004-33メッシュ結合解析オーケストレーション)・[FR-008](#fr-008-per-mesh-csv-統合)・[FR-018](#fr-018-per-mesh-ピーク候補-geojson-統合) へコマンドライン引数として転送する。省略時は各 FR のデフォルト（日本全土1次メッシュコードリスト）が使われる（本 FR は日本全土リストを直接読まない） |
...
| 1次メッシュコードリスト | ユーザー入力 | 任意 | 日本全土１次メッシュコードリスト | [7.1.2.1 参照](#7121-1次メッシュコードリスト) |
| 日本全土１次メッシュコードリスト | 内部データ | 必須 | — | [7.2.1 参照](#721-内部データ一覧) |
| 標高タイル（ローカルキャッシュ） | 内部データ | 必須 | — | [FR-001](#fr-001-標高タイル事前取得) が事前取得済みであること（保存先は [7.2.1 参照](#721-内部データ一覧)）。タイルが存在しない領域は NODATA として扱う |
...
| 日本全土１次メッシュコードリスト | 内部データ | 必須 | — | [7.2.1 参照](#721-内部データ一覧)。4 コーナーパターン生成時の存在メッシュ判定（無効パターンスキップ・bounding box 解析範囲決定）に使用 |
| 標高タイル（ローカルキャッシュ） | 内部データ | 必須 | — | [FR-001](#fr-001-標高タイル事前取得) が事前取得済みであること（保存先は [7.2.1 参照](#721-内部データ一覧)）。タイルが存在しない領域は NODATA として扱う |
...
| 陸地最高峰リスト | 内部データ | 必須 | — | [7.2.1 参照](#721-内部データ一覧)。富士山・旭岳・中岳＝九重の緯度経度リスト。`merged_peak.csv` 生成時に陸地最高峰の海面確定（`key_col_resolved=true`・Key コル = 0m）を適用する |
...
| 日本全土1次メッシュコードリスト | 内部データ | 必須 | — | `expected_count` 算出の基準母集合（[7.2.1 参照](#721-内部データ一覧)）。`対象1次メッシュコードリスト` で範囲を絞っても本リストは常に全土基準のまま使用する |
...
| 2 | 1次メッシュコードリスト | コマンドライン引数（単一コードまたはファイルパス） | [FR-001](#fr-001-標高タイル事前取得)・[FR-004](#fr-004-33メッシュ結合解析オーケストレーション)・[FR-008](#fr-008-per-mesh-csv-統合)・[FR-018](#fr-018-per-mesh-ピーク候補-geojson-統合)・[FR-023](#fr-023-解析パイプライン制御) | [7.1.2.1 参照](#7121-1次メッシュコードリスト) |
| 3 | HTML ビューア上のユーザー入力 | ブラウザ入力フォーム（山岳名・rationale） | [FR-019](#fr-019-html-ビューア機能仕様) | [7.1.2.2 参照](#7122-html-ビューア上のユーザー入力) |

#### 7.1.2 ユーザー入力詳細仕様

##### 7.1.2.1 1次メッシュコードリスト
...
##### 7.1.2.2 HTML ビューア上のユーザー入力

| 項目 | 仕様 |
|---|---|
| 用途 | 申請書・サミット一覧（申請内容反映版）に転記する山岳名と、申請根拠（rationale）の編集 |
| 入力フィールド | 山岳名（日本語）・山岳名（英語）・rationale（各ピークの申請根拠テキスト） |
| 管理 FR | [FR-019](#fr-019-html-ビューア機能仕様) が入力フィールドを制御し、[7.2.2.2](#7222-localstorage-編集内容詳細仕様) に永続化する |

##### 7.1.2.3 N03 行政区域 ZIP
...
##### 7.2.2.2 localStorage 編集内容詳細仕様

| 項目 | 仕様 |
|---|---|
| 用途 | HTML ビューア（[7.1.2.2](#7122-html-ビューア上のユーザー入力)）上の入力・編集内容をブラウザセッション間で保持する |
...
- 以下の参照データはツールが自動取得しない。ユーザーが手動で管理することが前提:
  - N03 行政区域 ZIP（[7.1.2.3 参照](#7123-n03-行政区域-zip)）— サイズが大きいため git 管理外。国土交通省 国土数値情報サイトから ZIP 形式で手動ダウンロードして `$DATA_DIR/ref/` 直下に配置する
