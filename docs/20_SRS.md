...
地形解析エンジン         FR-014（広域結合解析オーケストレーション・N×N + L14・コル確定のみ）
...
- **概要**: コル未確定ピーク（独立峰）に対する広域メッシュ結合解析のオーケストレーション。各対象ピークについて N×N メッシュ（N=4/5/6・ズームレベル14）の **4 コーナー配置パターン**（NW/NE/SW/SE）を内部生成し、per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）を呼び出してコル確定を試みる。広域 per-mesh CSV を新規出力し、[FR-008](#fr-008-per-mesh-csv-統合) の再実行で `merged_peak.csv` に統合する。

**入力**:

| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| コル未確定ピーク座標リスト | 内部データ | 必須 | — | [FR-022](#fr-022-コル充足判定) が `merged_peak.csv` から抽出した `key_col_resolved=false` のピーク群（緯度経度） |
| 解析範囲サイズ N | ユーザー入力 | 必須 | — | 4・5・6 のいずれか。[FR-022](#fr-022-コル充足判定) の N エスカレーション制御により段階的に渡される（[データ辞書参照](#221-設定可能項目)） |
| 1次メッシュコードリスト | ユーザー入力 | 必須 | — | 無効パターンスキップ判定（存在しないメッシュコードを含むパターンの除外）に使用 |

**出力**:

| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 広域 per-mesh ピーク候補 CSV | 内部データ | CSV。ファイル: `$DATA_DIR/results/csv/<解析識別子>.csv`（例: `4-5239-NW.csv`）、エンコーディング: UTF-8 | 対象ピーク座標リストに含まれるピーク行のみを出力する。[FR-008](#fr-008-per-mesh-csv-統合) の再実行で `merged_peak.csv` に統合される |

**説明**:

  - **解析識別子**: 本 FR は各コーナーパターンの処理開始時に `<N>-<対象ピーク所属メッシュコード>-<コーナー>`（例: `4-5239-NW`）形式の解析識別子を生成し、[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) に内部トランザクションとして渡す。通常 per-mesh（`3-<meshcode>`）と広域 per-mesh（`4-<meshcode>-NW` 等）はプレフィックスで機械的に区別できる
  ...
  - **解析範囲標高グリッドの生成**: コーナー配置パターンごとに、[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と同様の手順でタイル結合・標高デコード・1px 海面ボーダー付加を行い、解析範囲標高グリッドを生成する（ズームレベル14・max pooling 適用）
  - **per-mesh パイプライン呼び出し**: 各パターンの解析範囲標高グリッドに対し、per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）を呼び出してピーク候補検出・コル検出・CSV 出力を実行する。[FR-016](#fr-016-ピーク域ポリゴン生成) は呼び出さない（広域モードの目的はコル特定のみで、ポリゴンは通常 per-mesh の結果を使用する）
  - **早期終了**: 各対象ピークについて 4 パターンを順に試し、当該ピークが `key_col_resolved=true` を得たパターンが見つかった時点で、そのピークについての残りのコーナーパターンはスキップする
  - **無効パターンのスキップ**: 存在しないメッシュ（海域・国外等、1次メッシュコードリスト未存在）を含むパターンは試行せずスキップする
  - **対象ピーク絞り込み出力**: 解析範囲内で検出された全ピークではなく、対象ピーク座標リストに含まれるピーク行のみを per-mesh CSV に出力する（[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) 出力動作参照）
  - per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）は [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と本 FR の双方から呼び出される共通サブ処理である。ピーク検出・コル検出ロジック自体は N の値・ズームレベルによらず共通
...
**異常系**: 4 コーナー全パターンを試行しても対象ピークの `key_col_resolved` が確定しない場合、当該ピークは `key_col_resolved=false` のまま広域 per-mesh CSV に出力する（[FR-022](#fr-022-コル充足判定) が次の N または最終残存判断を担当）。
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 解析範囲標高グリッド | 内部トランザクション | 必須 | — | [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) の出力 |
...
**説明**:
  ...
  - 解析範囲標高グリッド（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) のタイル読み込み完了直後）を入力とし、解析処理（[FR-005](#fr-005-ピーク候補検出)）の実行前に色分け PNG を生成する
...
|---|---|---|---|---|
| コル情報 | 内部データ | 必須 | — | — |
| 解析対象メッシュ範囲情報 | 内部トランザクション | 必須 | — | [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) の出力。地理的範囲フィルタの範囲判定に使用 |
...
**説明**:
  ...
  - **未充足ループ（N=4→5→6 のエスカレーション）**:
    1. 抽出した未確定ピーク座標リストと現在の N（および 1次メッシュコードリスト）をフェーズ2-2（[FR-014](#fr-014-広域結合解析オーケストレーション)）に渡して呼び出す。FR-014 が内部で 4 コーナーパターン（NW/NE/SW/SE）の生成・解析・早期終了を行い、広域 per-mesh CSV を出力する
...
| ファイル | `$DATA_DIR/images/<解析識別子>_terrain.png`（例: `3-5239_terrain.png`） |
| 形式 | PNG |
| 解像度 | 長辺 6000px に縮小（アスペクト比保持、最近傍サンプリング）（参照: [ADR-SRS-012](decisions/ADR-SRS-012-terrain-image-downscaling-method.md)）|
| 生成タイミング | [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) の処理完了後、[FR-005](#fr-005-ピーク候補検出) 開始前 |
