...
- **対応 UR**: [UR-001](10_URD.md#ur-001), [UR-002](10_URD.md#ur-002)
- **概要**: 解析範囲標高グリッドの全ピクセルを標高降順に走査し、8 近傍に処理済みピクセルが無いピクセルを新規ピーク候補とする。
...
**説明**:
  ...
  - **海面確定規則**: 山塊グループが解析範囲端（1px 海面ボーダー）に一度も接触せず flood が完結した場合、そのピークは解析範囲内の陸地全体で最高点（島の最高峰または解析窓内で陸地が孤立した独立峰）と判断し、Key コル = 海面（0m）・`key_col_resolved=true` とする（プロミネンス = ピーク標高）。島の最高峰はこの規則で 3×3 または広域解析内で自動確定する
