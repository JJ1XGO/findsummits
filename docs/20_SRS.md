...
ただし内部トランザクション（FR の入出力として一時的に発生し、独立した中央データ構造として保持されない値）は一覧管理の対象外とし、発生する FR の入出力欄に発生元 FR を明示する。定義は [`00_GLOSSARY.md`「システム境界・データ分類」](00_GLOSSARY.md#システム境界データ分類) を参照（根拠: [ADR-SRS-017](decisions/ADR-SRS-017-internal-transaction-category.md)）。
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 標高タイル（ローカルキャッシュ） | 内部データ | PNG（DEM 種別・ズームレベル・タイル座標ごとに1ファイル） | ローカルキャッシュ保存先は [7.2 参照](#72-内部データ一覧) |
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| ピーク候補リスト | 内部データ | メモリ上のデータ構造（連結成分管理） | [FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぐ。各ピーク候補の連結成分情報を含む（連結成分は Union-Find で管理。詳細は HLD 参照） |
...
**説明**:
  ...
  - [FR-005](#fr-005-ピーク候補検出) の処理中に 2 つ以上の連結成分が初めて接触した時点のピクセルがコルであり、その標高がコル標高となる
  - プロミネンス = ピーク標高 − コル標高
  - コルが解析範囲外の場合、プロミネンスは暫定値とし、per-mesh CSV に `key_col_resolved=false` を付与する（[FR-022 参照](#fr-022-コル充足判定)）
  - **海面確定規則**: 連結成分が解析範囲端（1px 海面ボーダー）に一度も接触せず flood が完結した場合、そのピークは解析範囲内の陸地全体で最高点（島の最高峰または解析窓内で陸地が孤立した独立峰）と判断し、Key コル = 海面（0m）・`key_col_resolved=true` とする（プロミネンス = ピーク標高）。島の最高峰はこの規則で 3×3 または広域解析内で自動確定する
...
**説明**:
  ...
  - **地理的範囲フィルタ**: 解析範囲標高グリッドはタイル境界の丸めによりメッシュ境界より数十 m はみ出す（[FR-004 参照](#fr-004-33メッシュ結合解析オーケストレーション)）。このはみ出し（フリンジ）部分で検出されたピークは出力対象外とし、解析対象メッシュ全体の地理的範囲内のピークのみを出力する（範囲は解析対象メッシュ範囲情報から判定する）
  - **出力カラム**（ヘッダー行あり）:
...
| col_margin_px | int | — | コルから解析範囲（解析範囲標高グリッド）の端（4辺）までの最短距離（ピクセル単位）。小さいほど信頼性が低い |
| center_mesh | int | — | 解析中心メッシュコード（4桁） |
| zoom_level | int | — | 解析に使用したタイルズームレベル（通常モード=15、広域モード=14）。L14 の 1px は L15 の 2px 相当のため、[FR-008](#fr-008-per-mesh-csv-統合) の stability 判定時に `col_margin_px` を L15 相当へ換算する（[ADR-SRS-004](decisions/ADR-SRS-004-level14-max-pooling-isolated-peaks.md)） |
  ...
  - **広域モード（フェーズ2-2・[FR-014](#fr-014-広域結合解析オーケストレーション) 呼び出し時）の出力動作**:
    ...
    - **ファイル命名**: [FR-014](#fr-014-広域結合解析オーケストレーション) から渡された解析識別子（`<N>-<メッシュコード>-<コーナー>` 形式。例: `4-5239-NW`）をファイル名の基底として使用する。通常 per-mesh（`3-<meshcode>.csv`）と広域 per-mesh（`4-<meshcode>-NW.csv` 等）は解析識別子のプレフィックスで機械的に区別できる
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| per-mesh アクティベーションゾーン GeoJSON | 内部データ | GeoJSON（2種類のポリゴンを同一ファイルに収録）。ファイル: `$DATA_DIR/results/csv/<解析識別子>_activation.geojson`、形式: GeoJSON（RFC 7946）、座標参照系: WGS84（EPSG:4326）。プロパティ仕様は下記「詳細」参照 | — |
**説明**:
...
**説明**:
  ...
  - **通常モード（フェーズ2-1）の per-mesh CSV** と **広域モード（フェーズ2-2・[FR-014](#fr-014-広域結合解析オーケストレーション) で生成）の per-mesh CSV** が同一ディレクトリ配下に混在することがある。両方を読み込み、同一ピーク座標で重複排除する
...
内部トランザクション（FR の入出力として一時的に発生し、独立した中央データ構造として保持されない値）は本一覧の対象外。各 FR の入出力欄に発生元 FR を明示する（根拠: [ADR-SRS-017](decisions/ADR-SRS-017-internal-transaction-category.md)）。

| データ名 | 保存先 / 形態 | 生成 FR | 参照 FR | 補足 |
|---|---|---|---|---|
| ピーク候補リスト | メモリ上 | [FR-005](#fr-005-ピーク候補検出) | [FR-006](#fr-006-コル検出プロミネンス計算) / [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) / [FR-016](#fr-016-ピーク域ポリゴン生成) | 連結成分（Union-Find 構造）を含む |
...
| 標高タイル（ローカルキャッシュ） | `$DATA_DIR/tiles/{z}/{x}/{y}_{suffix}.png`<br>`{suffix}`: `a`=DEM5a / `b`=DEM5b・DEM10b / `c`=DEM5c | [FR-001](#fr-001-標高タイル事前取得) | [FR-002](#fr-002-dem-階層フォールバック) / [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) | |
