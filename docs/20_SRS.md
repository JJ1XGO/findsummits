...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| ピーク候補リスト | 内部トランザクション | ピーク候補のリスト | [FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぐ。各ピーク候補の連結成分情報を含む（連結成分は Union-Find で管理。詳細は HLD 参照） |
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| コル情報 | 内部トランザクション | コル情報のリスト | [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぐ。コル座標・コル標高・プロミネンス・`key_col_resolved` フラグを含む |
