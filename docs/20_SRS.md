...
この SRS の機能要件（FR-XXX）でパラメータを参照するときは、本セクションで定義した名称を使用すること。外部I/F は [6. 外部インターフェース仕様](#6-外部インターフェース仕様)、ユーザー入力は [7. ユーザー入力](#7-ユーザー入力)、内部データは [8. 内部データ](#8-内部データ) で定義した名称を使用すること。定義の重複を防ぎ、変更時の更新箇所を一元化するためのセクションである（[ADR-SRS-016](decisions/ADR-SRS-016-data-classification-external-user-internal.md) 参照）。
...
| タイル取得のリクエスト間隔 | タイル取得リクエスト間の最短待機時間（詳細は HLD で定義） | 100ms | 0〜5000ms |
...
- **対応 UR**: [UR-006](10_URD.md#ur-006), [UR-014](10_URD.md#ur-014)（描画応答性・マーカー描画対象）
...
- **対応 UR**: [UR-006](10_URD.md#ur-006), [UR-011](10_URD.md#ur-011), [UR-014](10_URD.md#ur-014)（描画応答性・マーカー描画対象）
...
画面サンプル: [`docs/mockup/viewer_mockup.html`](mockup/viewer_mockup.html)（作業用ビューアと共通のモックアップ。公開用は編集 UI・XLSX エクスポートなし）。モックアップの位置付け（UI 確定用プロトタイプ・正は仕様書）は [6.2.5](#625-作業用-html-ビューア) 注記および [ADR-SRS-039](decisions/ADR-SRS-039-mockup-positioning-and-spec-reflection.md) を参照
