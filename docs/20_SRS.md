...
| delete_zone 比高上限 | delete判定ゾーン Flood Fill の比高上限。Key コルとの標高差と本値の小さい方が Flood Fill の下限閾値となる（変更は ADR 判断を要する。デフォルト 250m は解析実測値） | 250m | 200〜300m |
...
- **外部データソース**
  ...
  - 背景タイル（国土地理院標準地図・淡色地図・OSM・OpenTopoMap、ブラウザから実行時取得）
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 1次メッシュコードリスト | ユーザー入力 | 任意 | 日本全土１次メッシュコードリスト | [7.2.1 参照](#721-1次メッシュコードリスト) |
| 日本全土１次メッシュコードリスト | 内部データ | 必須 | — | [8.1 参照](#81-内部データ一覧) |
...
**説明**:
  ...
  - **解析対象メッシュの決定**: 対象メッシュを中心とした 3×3 のメッシュグリッド（最大 9 メッシュ）のうち、1次メッシュコードリスト（省略時は日本全土１次メッシュコードリスト）に存在するものを解析対象メッシュとする。
  ...
  - **オーケストレーション（1 メッシュあたりの処理順序）**: 解析範囲標高グリッド 1 件につき、以下を順に呼び出す
    ...
    ```text
    解析範囲標高グリッドの生成（タイル結合 + [FR-002](#fr-002-dem-階層フォールバック)/[FR-003](#fr-003-標高デコードnodata-処理) 適用 + 海面ボーダー付加）
      ↓
    [FR-015](#fr-015-標高地形図出力): 標高地形図出力（解析実行前の読み込み正常性確認用 PNG）
      ↓
    [FR-005](#fr-005-ピーク候補検出): ピーク候補検出 / [FR-006](#fr-006-コル検出プロミネンス計算): コル検出・プロミネンス計算
            （同一の標高降順ピクセル走査の中で実行される）
      ↓
    [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用): per-mesh CSV 出力（プロミネンス一次フィルタ・地理的範囲フィルタ適用）
      ↓ [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) のフィルタ後ピーク候補・コル情報リストを [FR-016](#fr-016-ピーク域ポリゴン生成) へ渡す（※ 解析範囲標高グリッド・解析対象メッシュ範囲情報・解析識別子も同時に引き渡す）
    [FR-016](#fr-016-ピーク域ポリゴン生成): ピーク域ポリゴン生成（per-mesh ピーク候補 GeoJSON 出力）
      ↓
...
| 日本全土１次メッシュコードリスト | 内部データ | 必須 | — | [8.1 参照](#81-内部データ一覧)。4 コーナーパターン生成時の存在メッシュ判定（無効パターンスキップ・bounding box 解析範囲決定）に使用 |
| 標高タイル（ローカルキャッシュ） | 内部データ | 必須 | — | [FR-001](#fr-001-標高タイル事前取得) が事前取得済みであること（保存先は [8.1 参照](#81-内部データ一覧)）。タイルが存在しない領域は NODATA として扱う |
...
**説明**:
  ...
  - **対象メッシュへのグルーピング**: コル未確定ピーク座標リストの各ピーク座標から所属する 1次メッシュコードを算出し（[メッシュコード → 緯度経度変換](00_GLOSSARY.md#mesh-to-latlon) の逆変換）、同一メッシュに所属するピークを 1 グループにまとめる。以降の処理（4 コーナーパターン生成〜早期終了）は対象メッシュ（グループ）単位で実行する（同一ウィンドウの重複解析と出力ファイル名の衝突を防ぐ）。複数グループが存在する場合はメッシュコード昇順で処理する（処理順序の再現性確保のため。順序は解析結果に影響しない）
...
**説明**:
  ...
  - **マイナス標高の扱い**: NODATA 判定（前項：x = 0x800000）を符号変換より先に適用するため、デコード式の中間値 x（符号なし 24bit）が 0x800000（= 2^23）より大きい場合（x = 0x800001 以上）は `x - 2^24` を適用して符号付き整数に変換し、100 で除算する。これにより干拓地など海面下ピクセルは負の標高値としてデコードされる。ただしデコード完了後のタイル処理段階で、負値は無効値（-9999m）と同様に海面高度（`ELEV_SEA = 0.0m`）に統一される。この時点で「本来の海面（0m）」「NODATA」「負の標高」は区別されなくなる
...
| 10 | 背景タイル（国土地理院標準地図・淡色地図・OSM・OpenTopoMap） | 入力 | PNG | [FR-019](#fr-019-html-ビューア機能仕様) | ブラウザから実行時取得。基図としていずれか1つを常時表示。出典・利用形態: [SOURCES.md](../ref/SOURCES.md) |
...
- 正規化: `vmin = -t0・vmax/(1-t0)`、`t0=0.25`。`vmax` は解析範囲標高グリッドの実測最大標高
  （メッシュ毎の相対値。固定値ではない）
- 海面・負標高・NODATA はすべて [FR-003](#fr-003-標高デコードnodata-処理) で 0m に統一されるため、
  0m 以下を白（`#FFFFFF`）でマスクして表示する（カラーマップ・陰影の対象外。NODATA の識別は不可）
...
| 取得方法 | ユーザーが手動ダウンロードして配置する（ツールは自動取得しない） |
...
| ライフサイクル | ブラウザのストレージ上に存在する限り保持。各エクスポート（申請書 XLSX・サミット一覧反映版・公開用 HTML・申請エビデンス ZIP）時にマージして反映する |
...
| 外部システム | 用途 |
|---|---|
| 国土地理院 タイル配信（`cyberjapandata.gsi.go.jp`） | DEM5a/5b/5c/DEM10b タイル取得（[FR-001](#fr-001-標高タイル事前取得)）・HTML ビューアの背景地図（標準地図・淡色地図）・等高線オーバーレイ・基準点レイヤー（[FR-019](#fr-019-html-ビューア機能仕様)） |
