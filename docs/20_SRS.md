...
| 仮サミットコード連番上限 | 採番する仮コードの prefix の上限アルファベット（例: `A` なら A01〜A99 の 99 件、`Z` なら A00〜Z99 の最大 2,574 件まで許容）。上限を超えたら異常終了（[FR-009](#fr-009-sotaリスト突合match_status-判定) 参照） | A | A〜Z |
...
**説明**:
  ...
  - **座標変換**: 入力の「ピーク候補・コル情報リスト」のピーク座標はピクセル座標で渡される。CSV 出力時に解析に使用したズームレベル（通常モード=L15・広域モード=L14）に基づいて `peak_lat`/`peak_lon`（緯度経度・WGS84）へ変換する。コル座標（`col_lat`/`col_lon`）は [FR-006](#fr-006-コル検出プロミネンス計算) 出力時点で緯度経度変換済み。
  - **出力カラム**（ヘッダー行あり）:
