...
| 作成日 | 2026-04-30 |
| 最終更新日 | 2026-06-16 |
| ステータス | ドラフト（FR-018 レビュー反映 — FR-008 とセットでループ内再入・merged_peak.csv での絞り込み確定。ADR-SRS-024） |
...
| プロミネンス最終フィルタ閾値 | フェーズ3 で統合後に採用するピーク候補の最低プロミネンス | 150m | 150m（SOTA日本支部規定値） |
...
| 地形解析エンジン | DEM からピーク／コル／プロミネンス／AZ・delete判定ゾーンを検出 | ローカルキャッシュ済み PNG タイル | 処理モードにより異なる（通常モード: per-mesh CSV / per-mesh activation GeoJSON / 標高地形図 PNG、広域モード: per-mesh CSV のみ） |
...
地形解析エンジン         FR-004（3×3 通常モード）: ピーク・コル検出・ピーク域ポリゴン生成
       ...
       ├─ per-peak polygons         ($DATA_DIR/results/csv/3-<meshcode>_activation.geojson)
...
地形解析エンジン         FR-014（広域結合解析オーケストレーション・N×N + L14・ポリゴン生成なし）
       ...
       ↓ → FR-008（per-mesh CSV 統合）+ FR-018（per-mesh activation.geojson 統合）をセットで再実行 → コル充足判定へ戻る
...
**説明**:
  ...
  - ローカルキャッシュ済みタイルの再取得時は `If-Modified-Since` ヘッダーを付与してリクエストし、サーバー側に更新がない場合（HTTP 304）はダウンロードをスキップする
...
全解析対象メッシュを 3×3 の通常モード（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション)）で処理し、per-mesh CSV・アクティベーションゾーン GeoJSON・標高地形図を生成する。コル未確定ピークが残った場合はフェーズ2-2 の広域モード（[FR-014](#fr-014-広域結合解析オーケストレーション)）で広域 per-mesh CSV・標高地形図を生成する。
...
**説明**:
  ...
  - **オーケストレーション（1 メッシュあたりの処理順序）**: 解析範囲標高グリッド 1 件につき、以下を順に呼び出す
    ...
    FR-016: ピーク域ポリゴン生成（per-mesh アクティベーションゾーン GeoJSON 出力）
    ```

  - per-mesh CSV に出力されるピークは解析対象メッシュ全体の地理的範囲内のもの（中心メッシュ内に限定しない）。範囲判定は [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) の地理的範囲フィルタが行う
  - メッシュコードリストの全メッシュを逐次実行すると、同一ピークが複数メッシュの解析（最大 9 回）に含まれ、複数の per-mesh CSV に重複して出力される。フェーズ3 で重複排除し、`analysis_count`（実際の解析回数）と `expected_count`（期待解析回数）で安定性を評価する（[FR-008 参照](#fr-008-per-mesh-csv-統合)）
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| コル未確定ピーク座標リスト | 内部データ | 必須 | — | [FR-022](#fr-022-コル充足判定) が生成する `key_col_unresolved_peaks-<N>.csv`（`key_col_resolved=false` のピーク群・緯度経度）。ファイルとして渡される |
| 解析範囲サイズ N | 内部トランザクション | 必須 | — | 4・5・6 のいずれか。[FR-022](#fr-022-コル充足判定) がエスカレーションループの現段階 N を算出（`key_col_unresolved_peaks-<N>.csv` のファイル名から N を抽出し N+1 を渡す）して渡す |
...
- トリガー: [FR-022](#fr-022-コル充足判定) で `key_col_resolved=false` ピークが検出された場合のみ。`key_col_resolved=false` は、Key コルが 3×3 解析範囲内で見つからなかった独立峰（Key コルが解析範囲外に存在する）を意味する
- N エスカレーション（N=4→5→6）の制御は [FR-022](#fr-022-コル充足判定) が担う。本 FR は単一の N で呼び出される
...
- 対象メッシュの 3×3 相当範囲（フェーズ2-1 で解析済みの範囲）にタイルが 1 枚も存在しない場合、エラーを出力して処理を中止する（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と同じフェイルファースト方針。タイル事前取得（[FR-001](#fr-001-標高タイル事前取得)）の未実行・取得漏れが疑われるため、原因の確認と再実行をユーザーに促す）
...
- 4 コーナー全パターンを試行しても対象ピークの `key_col_resolved` が確定しない場合、当該ピークは `key_col_resolved=false` のまま広域 per-mesh CSV に出力する（[FR-022](#fr-022-コル充足判定) が次の N または最終残存判断を担当）
...
**説明**:
  ...
  - フェーズ2 で検出したピーク候補とコルの情報を per-mesh CSV として出力する。フェーズ3（[FR-008](#fr-008-per-mesh-csv-統合)）での最終判定（**プロミネンス最終フィルタ閾値**以上）に備えて以下のフィルタを適用する:
...
| analysis_id | str | — | 解析識別子。通常モード: `3-<中心メッシュコード>`（例: `3-5239`）、広域モード: `<N>-<対象メッシュコード>-<コーナー>`（例: `4-5239-NW`）。mode・meshcode・corner・zoom の全情報を内包する（[ADR-SRS-021](decisions/ADR-SRS-021-per-mesh-csv-column-design.md) 参照） |
  ...
  - **広域モード（フェーズ2-2・[FR-014](#fr-014-広域結合解析オーケストレーション) 呼び出し時）の出力動作**:
...
per-mesh 出力（通常モード・広域モード）を全国スケールで統合し、コル充足判定でフェーズ4 移行可否を決める。全ピークのコルが確定するまでフェーズ2-2（[FR-014](#fr-014-広域結合解析オーケストレーション)）への要求とフェーズ3 再実行を繰り返す（広域解析の実現方式は [FR-014](#fr-014-広域結合解析オーケストレーション) を参照）。
...
**説明**:
  ...
  - **統合対象ルール**: `対象1次メッシュコードリスト` が指定された場合、統合対象は指定メッシュコードの**通常 per-mesh CSV のみ**（広域 per-mesh CSV は除外）。未指定（デフォルト=日本全土）の場合は、**通常モード（フェーズ2-1）の per-mesh CSV** と **広域モード（フェーズ2-2・[FR-014](#fr-014-広域結合解析オーケストレーション) で生成）の per-mesh CSV** の両方を対象とする。両者が同一ディレクトリ配下に混在する場合も、`analysis_id` の接頭辞（`3` = 通常・`4/5/6` = 広域）で機械的に判別する。なお [FR-022](#fr-022-コル充足判定) のエスカレーションループ内から再実行される場合は `対象1次メッシュコードリスト` を渡さない（全土デフォルトで通常+広域を統合する）
  - 同一ピーク座標（ズームレベル15 タイル座標が一致）のレコードを同一ピークとして重複排除し、統合する。広域モードの per-mesh CSV もピーク座標をズームレベル15 座標で出力するため（[FR-014](#fr-014-広域結合解析オーケストレーション) 参照）、本判定は通常・広域の区別なく機能する
    - **代表採用ロジック**（重複した複数解析レコードから 1 件を選ぶ優先順位。決定論的に一意となるよう以下の順で適用する）:
      ...
      4. 全レコードが `key_col_resolved=false` の場合は `key_col_resolved=false` のまま維持する（プロミネンス未確定のため、[FR-022](#fr-022-コル充足判定) がエスカレーション対象として抽出する）
  ...
  - **再入可能性**: 本機能はコル充足判定（[FR-022](#fr-022-コル充足判定)）のループから複数回呼び出され、その都度 merged_peak.csv（内部 work CSV）が再生成される。詳細・設計判断: [ADR-SRS-023](decisions/ADR-SRS-023-fr008-merge-input-mesh-list-semantics.md)
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| per-mesh アクティベーションゾーン GeoJSON | 内部データ | 必須 | — | 通常 per-mesh のみ（広域モード = [FR-014](#fr-014-広域結合解析オーケストレーション) は GeoJSON を生成しない） |
...
- **対応 UR**: [UR-007](10_URD.md#ur-007)
- **概要**: `merged_peak.csv` の全ピークがコル確定（`key_col_resolved=true`）であるかを判定し、フェーズ4 への移行可否を決める。未確定ピークがある場合は N=4→5→6 のエスカレーションでフェーズ2-2（[FR-014](#fr-014-広域結合解析オーケストレーション)）を呼び出し、`merged_peak.csv` を更新する（[FR-014](#fr-014-広域結合解析オーケストレーション) の実現方式参照）。
...
| 陸地最高峰リスト | ユーザー入力 | 必須 | — | [7.1 参照](#714-陸地最高峰リスト)。どのウィンドウでも海面確定できない陸地最高峰（富士山・旭岳・中岳＝九重）の緯度経度リスト |
...
| コル未確定ピーク座標リスト（`key_col_unresolved_peaks-<N>.csv`） | 内部データ | CSV | N = 解析済み段階（3×3 後は `key_col_unresolved_peaks-3.csv`、4×4 後は `-4.csv`）。次回エスカレーション時に [FR-014](#fr-014-広域結合解析オーケストレーション) に渡す |
...
**説明**:
  ...
  - **陸地最高峰の処理（ループ前に実施）**: 抽出した未確定ピークのうち、陸地最高峰リストと近傍一致（許容距離は HLD で定義）するピークは広域解析対象から除外し、`key_col_resolved=true`（Key コル = 海面 0m）に更新する（どのウィンドウ配置でも陸地全体を囲めないため、[FR-006 海面確定規則](#fr-006-コル検出プロミネンス計算) を手動適用する形式を採る）。更新後に `merged_peak.csv` を再生成する
  - **未充足ループ（N=4→5→6 のエスカレーション）**:
    1. 陸地最高峰除外後に残った未確定ピークの座標リストを `key_col_unresolved_peaks-<現在の N>.csv` として出力する（N = 前段解析の N: フェーズ2-1 完了時は N=3、4×4 広域解析完了後は N=4 等）
    2. 次段の解析範囲サイズ（現在の N+1）と `key_col_unresolved_peaks-<N>.csv` のファイルパスをフェーズ2-2（[FR-014](#fr-014-広域結合解析オーケストレーション)）に渡して呼び出す。FR-014 が内部で 4 コーナーパターン（NW/NE/SW/SE）の生成・解析・早期終了を行い、広域 per-mesh CSV を出力する
    3. [FR-008](#fr-008-per-mesh-csv-統合) を再実行して `merged_peak.csv` を再生成する（`対象1次メッシュコードリスト` は渡さず全土デフォルトで動作させ、通常 per-mesh CSV と広域 per-mesh CSV の両方を統合対象とする。`key_col_resolved=true` のレコードがあれば代表として採用する）
    4. 更新後の `merged_peak.csv` を再判定する
    5. まだ未確定が残れば N を +1 して 1 を繰り返す（N の上限は 6）
...
| 1次メッシュコードリスト | ユーザー入力 | 任意 | 未指定時=全範囲対象。[FR-010](#fr-010-削除候補のスコープ) の削除候補スコープ判定に使用 | — |
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 1次メッシュコードリスト | ユーザー入力 | 任意 | 未指定時は全 SOTA サミットを削除候補対象とする（全国フル解析を前提） | FR-009 に渡されたものと同じリスト |
...
| `unchanged.geojson` | 変更なし既存サミット | peak の `match_status="matched"` かつ `is_band_change_candidate=false` ＋ 対応する summit フィーチャ |
  ...
  - 各 GeoJSON には `metadata`（`summitslist_date` / `gsi_tile_latest_date` / `generated_at` / `attribution` / `source_url` / `license_url`）を複製する（`attribution` 等は [UR-011](10_URD.md#ur-011) 準拠の固定値。詳細は [FR-013 メタデータ定義](#fr-013-html-ビューア生成) を参照）
...
| 使用 FR | [FR-001](#fr-001-標高タイル事前取得)・[FR-004](#fr-004-33メッシュ結合解析オーケストレーション)・[FR-008](#fr-008-per-mesh-csv-統合)・[FR-018](#fr-018-per-mesh-activationgeojson-統合)・[FR-009](#fr-009-sotaリスト突合match_status-判定) が受け取る |
...
| 項目 | 仕様 |
|---|---|
| 用途 | [FR-022](#fr-022-コル充足判定) が広域解析ループ前に陸地最高峰を海面確定（`key_col_resolved=true`・Key コル = 0m）に更新するための既知リスト |
| 対象ピーク | どのウィンドウ配置（N=4〜6 の 4 コーナー）でも陸地全体を囲むことができない陸地最高峰。初期リスト: 富士山（本州）・旭岳（北海道）・中岳＝九重（九州）の 3 件 |
| 形式 | テキストファイル（1行1件。ヘッダー行あり。列: `name,peak_lat,peak_lon`） |
| 配置場所 | `params/` 直下（ファイル名は HLD で定義） |
| 検証方針 | 石鎚山（四国）が実データで海面確定規則により 3×3 で自動確定するかを実装後に確認し、確定しない場合はリストに追加する（[ADR-SRS-019](decisions/ADR-SRS-019-land-summit-highest-peak-handling.md) 参照） |
...
| per-mesh アクティベーションゾーン GeoJSON | 一時ファイル。ファイル名: `$DATA_DIR/results/csv/3-<meshcode>_activation.geojson` | [FR-016](#fr-016-ピーク域ポリゴン生成) | [FR-018](#fr-018-per-mesh-activationgeojson-統合) | AZ + delete 判定ゾーン。通常 per-mesh のみ（広域モードは GeoJSON を生成しない） |
| 統合ピーク候補 work CSV（`merged_peak.csv`） | 内部 work ファイル。ファイル名: `$DATA_DIR/results/merged_peak.csv` | [FR-008](#fr-008-per-mesh-csv-統合) | [FR-009](#fr-009-sotaリスト突合match_status-判定) / [FR-022](#fr-022-コル充足判定) | |
| コル未確定ピーク座標リスト（`key_col_unresolved_peaks-<N>.csv`） | 一時ファイル。ファイル名: `key_col_unresolved_peaks-<N>.csv`（N = 解析済み段階：3×3 後は `-3`、4×4 後は `-4`） | [FR-022](#fr-022-コル充足判定) | [FR-014](#fr-014-広域結合解析オーケストレーション) | FR-022 がエスカレーションループの各段階で生成し、次段の FR-014 呼び出し時に渡す |
