...
この SRS の機能要件（FR-XXX）でパラメータを参照するときは、本セクションで定義した名称を使用すること。外部I/F は [6. 外部インターフェース仕様](#6-外部インターフェース仕様)、ユーザー入力は [7.1 ユーザー入力一覧](#71-ユーザー入力一覧)、内部データは [7.2 内部データ](#72-内部データ) で定義した名称を使用すること。定義の重複を防ぎ、変更時の更新箇所を一元化するためのセクションである（[ADR-SRS-016](decisions/ADR-SRS-016-data-classification-external-user-internal.md) 参照）。
