...
| col_margin_px | int | — | コルから解析範囲（解析範囲標高グリッド）の端（4辺）までの最短距離（ピクセル単位）。診断・将来用フィールド（[ADR-SRS-020](decisions/ADR-SRS-020-peak-col-pair-record.md) 参照）。`key_col_resolved=false` の場合は小さい値になる傾向がある |
| center_mesh | int | — | 解析中心メッシュコード（4桁） |
| zoom_level | int | — | 解析に使用したタイルズームレベル（通常モード=15、広域モード=14） |
