...
**説明**:
  ...
  - **異常系**: 該当なし。ブラウザ内のUI・インタラクション仕様であり、入力は埋め込み済みデータのみ。入力値の妥当性検証（不正な座標入力等）・localStorage容量超過時の異常時挙動はHLDに委ねる。それ以外に失敗しうる外部境界は持たない
...
`no_change`・`review` は XLSX 行を出力しない（`no_change` は変更申請不要。`review` は担当者が手動判断）。名称変更・座標変更等（[UR-004](10_URD.md#ur-004) の「その他」アクション）は自動識別対象外（[UR-003](10_URD.md#ur-003)）のためエクスポート対象外。
  ...
  - **異常系**: 該当なし。HTMLビューア内のブラウザ処理（SheetJSによるXLSX生成）であり、入力GeoJSONは常に有効な状態でビューアに埋め込み済みのため失敗しうる外部境界を持たない
...
- **異常系**: 該当なし。FR-011と同様、ブラウザ内の決定論的なXLSX生成処理であり、入力は埋め込み済みデータのみ。失敗しうる外部境界を持たない
...
| `review.geojson` | 要確認サミット（孤立既存サミット） | `category="review"` のフィーチャ全て（[ADR-SRS-037](decisions/ADR-SRS-037-unmatched-summit-needs-review.md)） |
  ...
  - **異常系**: 該当なし。HTMLビューア内のブラウザ処理（JSZipによるZIP生成）であり、入力は埋め込み済みデータのみ。失敗しうる外部境界を持たない
...
**説明**:
  ...
  - **異常系**: 該当なし。HTMLビューア内のブラウザ処理（自己完結型HTML生成）であり、入力は埋め込み済みデータのみ。失敗しうる外部境界を持たない
