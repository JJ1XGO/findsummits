...
- **概要**: メッシュコードリストの各メッシュを 3×3 単位（中心+隣接最大 8）で解析対象として解析範囲結合画像を生成し、標高地形図出力（[FR-015](#fr-015-標高地形図出力)）と per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)〜[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)・[FR-016](#fr-016-ピーク域ポリゴン生成)）を順に呼び出すオーケストレーション。3×3 通常モード専用。広域モード（N≥4）のオーケストレーションは [FR-014](#fr-014-広域結合解析オーケストレーション) を参照。
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 解析範囲結合画像 | 内部トランザクション | メモリ上 | 解析対象メッシュ範囲情報（下記説明参照）および解析識別子（下記説明参照）を伴う。後段の [FR-015](#fr-015-標高地形図出力)/[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き渡す |
...
**説明**:
  ...
  - **解析範囲結合画像の生成**: 特定したタイルから解析範囲結合画像を生成する。各ピクセルの標高値は、同座標における各 DEM 階層タイルの RGB から [FR-002](#fr-002-dem-階層フォールバック) で DEM 階層を選択し、[FR-003](#fr-003-標高デコードnodata-処理) でデコードして得る。タイルとメッシュの境界は一致しないため、解析範囲結合画像はメッシュ境界より数十 m はみ出す
    - タイルが存在しない領域（リスト外メッシュ・地理院側未整備タイル）は NODATA として充填する（具体例の 5140）
    - 結合範囲の外周に 1px 幅の海面ボーダー（0m）を付加し、メッシュ端を海岸線とみなす
  - **解析対象メッシュ範囲情報**: 解析対象メッシュのメッシュコード集合と、解析対象メッシュ全体の地理的範囲（緯度経度）↔ 解析範囲結合画像上のピクセル位置の対応を指す。後段の地理的範囲フィルタ（[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）・`area_complete` 判定（[FR-016](#fr-016-ピーク域ポリゴン生成)）が参照する
  - **オーケストレーション（1 メッシュあたりの処理順序）**: 解析範囲結合画像 1 枚につき、以下を順に呼び出す

    ```
    解析範囲結合画像の生成（タイル結合 + FR-002/FR-003 適用 + 海面ボーダー付加）
      ↓
...
**説明**:
  ...
  - **解析範囲結合画像の生成**: コーナー配置パターンごとに、[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と同様の手順でタイル結合・標高デコード・1px 海面ボーダー付加を行い、解析範囲結合画像を生成する（ズームレベル14・max pooling 適用）
  - **per-mesh パイプライン呼び出し**: 各パターンの解析範囲結合画像に対し、per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）を呼び出してピーク候補検出・コル検出・CSV 出力を実行する。[FR-016](#fr-016-ピーク域ポリゴン生成) は呼び出さない（広域モードの目的はコル特定のみで、ポリゴンは通常 per-mesh の結果を使用する）
...
- **概要**: 解析開始前に解析範囲結合画像から色分け PNG を生成し、解析対象範囲と地形の妥当性を目視確認できるようにする。

**入力**:

| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 解析範囲結合画像 | 内部トランザクション | 必須 | — | [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) の出力 |
...
**説明**:
  ...
  - 解析範囲結合画像（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) のタイル読み込み完了直後）を入力とし、解析処理（[FR-005](#fr-005-ピーク候補検出)）の実行前に色分け PNG を生成する
...
- **概要**: 解析範囲結合画像の全ピクセルを標高降順に走査し、8 近傍に処理済みピクセルが無いピクセルを新規ピーク候補とする。
...
**説明**:
  ...
  - **地理的範囲フィルタ**: 解析範囲結合画像はタイル境界の丸めによりメッシュ境界より数十 m はみ出す（[FR-004 参照](#fr-004-33メッシュ結合解析オーケストレーション)）。このはみ出し（フリンジ）部分で検出されたピークは出力対象外とし、解析対象メッシュ全体の地理的範囲内のピークのみを出力する（範囲は解析対象メッシュ範囲情報から判定する）
  - **出力カラム**（ヘッダー行あり）:
...
| col_margin_px | int | — | コルから解析範囲（解析範囲結合画像）の端（4辺）までの最短距離（ピクセル単位）。小さいほど信頼性が低い |
