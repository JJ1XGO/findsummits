...
**説明**:
  ...
  - **エラー伝播**: 呼び出した FR がエラー終了した場合は [FR-023](#fr-023-解析パイプライン制御) も即時終了する（フェイルファースト方針。`set -e` の使用等の具体的な実装手段は HLD に委ねる）
...
**説明**:
  ...
  - **走査開始前**: 海面（ELEV_SEA=0.0m）ピクセルおよび [FR-004](#fr-004-33メッシュ結合解析オーケストレーション)/[FR-014](#fr-014-広域結合解析オーケストレーション) が付加した外周 1px 海面ボーダーを含む全 0m ピクセルを、1 つの処理済み海面連結成分として初期化する。海面ピクセルはピーク候補にしない。この事前初期化により、走査ループ中に陸地連結成分が海面連結成分に 8 近傍接触した時点を「海面ボーダー接触」として [FR-006](#fr-006-コル検出プロミネンス計算) が検出できる
