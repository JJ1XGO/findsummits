...
| 地形解析エンジン | [FR-004](#fr-004-メッシュ結合解析) | メッシュ結合解析（3×3 通常モード） | | ✅ | | | |
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 1次メッシュコードリスト | ユーザー入力 | 必須 | — | [7.1.1 参照](#711-1次メッシュコードリスト) |
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 解析範囲結合画像 | 内部トランザクション | メモリ上 | 解析対象メッシュ範囲情報（下記説明参照）を伴う。後段の [FR-015](#fr-015-標高地形図出力)/[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き渡す |
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| 広域 per-mesh ピーク候補 CSV | 内部データ | CSV。通常 per-mesh と同一ディレクトリ（`$DATA_DIR/results/csv/`）に通常と区別できる命名で出力（具体的な命名規則は HLD 参照） | 対象ピーク座標リストに含まれるピーク行のみを出力する。[FR-008](#fr-008-per-mesh-csv-統合) の再実行で `merged_peak.csv` に統合される |
...
**説明**:
  ...
  - **解析範囲結合画像の生成**: コーナー配置パターンごとに、[FR-004](#fr-004-メッシュ結合解析) と同様の手順でタイル結合・標高デコード・1px 海面ボーダー付加を行い、解析範囲結合画像を生成する（ズームレベル14・max pooling 適用）
  ...
  - per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)/[FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)）は [FR-004](#fr-004-メッシュ結合解析) と本 FR の双方から呼び出される共通サブ処理である。ピーク検出・コル検出ロジック自体は N の値・ズームレベルによらず共通
...
| データ名 | 種別 | 必須/任意 | 備考 |
|---|---|---|---|
| DEM5a / 5b / 5c / 10b の同座標 RGB ピクセル値 | 内部トランザクション | 必須 | [FR-004](#fr-004-メッシュ結合解析) のタイル読み込みからピクセル単位で渡される |
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| デコード済みピクセル標高値 | 内部トランザクション | 浮動小数（m） | NODATA は -9999m、海面下は ELEV_SEA = 0.0m に統一。[FR-004](#fr-004-メッシュ結合解析) を経由して後段へ渡す |
...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| 解析範囲結合画像 | 内部トランザクション | 必須 | — | [FR-004](#fr-004-メッシュ結合解析) の出力 |
...
**説明**:
  ...
  - 解析範囲結合画像（[FR-004](#fr-004-メッシュ結合解析) のタイル読み込み完了直後）を入力とし、解析処理（[FR-005](#fr-005-ピーク候補検出)）の実行前に色分け PNG を生成する
...
**説明**:
  ...
  - **地理的範囲フィルタ**: 解析範囲結合画像はタイル境界の丸めによりメッシュ境界より数十 m はみ出す（[FR-004 参照](#fr-004-メッシュ結合解析)）。このはみ出し（フリンジ）部分で検出されたピークは出力対象外とし、解析対象メッシュ全体の地理的範囲内のピークのみを出力する（範囲は解析対象メッシュ範囲情報から判定する）
  - **出力カラム**（ヘッダー行あり）:
...
**説明**:
  ...
  - 通常 per-mesh の `<meshcode>_activation.geojson`（[FR-016](#fr-016-ピーク域ポリゴン生成) 出力）のみを統合対象とする。広域モード（[FR-014](#fr-014-広域結合解析オーケストレーション)）は GeoJSON を生成しないため、広域 per-mesh ファイルは本機能の入力に含まれない
...
| 広域 per-mesh ピーク候補 CSV | 一時ファイル | [FR-014](#fr-014-広域結合解析オーケストレーション)（[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) 経由） | [FR-008](#fr-008-per-mesh-csv-統合) | 命名規則は HLD |
...
| 標高タイル（ローカルキャッシュ） | `$DATA_DIR/tiles/{z}/{x}/{y}_{suffix}.png`<br>`{suffix}`: `a`=DEM5a / `b`=DEM5b・DEM10b / `c`=DEM5c | [FR-001](#fr-001-標高タイル事前取得) | [FR-002](#fr-002-dem-階層フォールバック) / [FR-004](#fr-004-メッシュ結合解析) | |
