...
- **対応 UR**: [UR-001](10_URD.md#ur-001), [UR-007](10_URD.md#ur-007)
- **概要**: メッシュコードリストの各メッシュを N×N 単位（中心+隣接最大 N²-1）で解析対象として解析範囲結合画像を生成し、per-mesh パイプライン（[FR-005](#fr-005-ピーク候補検出)〜[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)・[FR-016](#fr-016-ピーク域ポリゴン生成)）に引き渡すオーケストレーション。
...
| 解析範囲サイズ N | ユーザー入力 | 任意 | 3。フェーズ2-2（[FR-022](#fr-022-コル充足判定)）から呼び出された場合は N=4〜6 が渡される | [データ辞書参照](#221-設定可能項目) |
