...
海面・負標高・NODATA はすべて [FR-003](#fr-003-標高デコードnodata-処理) で 0m に統一されるため、0m 以下を -1m 扱い（沿岸色）で表示する（NODATA の識別は不可）。なお、-6000m ストップは Japan Topo カラースキームの下限定義として保持するが、0m 以下が -1m 統一される本実装では参照されない。

| 標高境界 (m) | RGB | 16 進 | 名称 |
|---|---|---|---|
| -6000 | (26, 79, 114) | #1a4f72 | 海洋（スキーム下限・本実装では未使用） |
