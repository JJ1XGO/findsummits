...
| データ名 | 種別 | 必須/任意 | デフォルト（任意時） | 備考 |
|---|---|---|---|---|
| コル未確定ピーク座標リスト | 内部データ | 必須 | — | [FR-022](#fr-022-コル充足判定) が `merged_peak.csv` から抽出した `key_col_resolved=false` のピーク群（緯度経度）。ファイルとして渡される |
| 解析範囲サイズ N | ユーザー入力 | 必須 | — | 4・5・6 のいずれか。[FR-022](#fr-022-コル充足判定) の N エスカレーション制御により段階的に渡される（[データ辞書参照](#221-設定可能項目)） |
...
**説明**:
  ...
  - **無効パターンのスキップ**: ウィンドウ内のメッシュのうち日本全土１次メッシュコードリストに存在しないもの（海域・国外等）は、タイル未取得領域として NODATA 充填し解析を続行する（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) のリスト外メッシュと同じ扱い）。ウィンドウ内の存在メッシュが対象メッシュの 3×3 解析範囲（フェーズ2-1 で解析済み）から 1 枚も増えないパターンは、解析しても新たな情報が得られないため試行せずスキップする
...
- 対象メッシュの 3×3 相当範囲（フェーズ2-1 で解析済みの範囲）にタイルが 1 枚も存在しない場合、エラーを出力して処理を中止する（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と同じフェイルファースト方針。タイル事前取得（[FR-001](#fr-001-標高タイル事前取得)）の未実行・取得漏れが疑われるため、原因の確認と再実行をユーザーに促す）。3×3 を超えて広域ウィンドウが初めて触れる範囲のタイル欠如は NODATA 充填で続行する（広域解析の性質上、事前取得範囲の外に及ぶことは正常）
...
| zoom_level | int | — | 解析に使用したタイルズームレベル（通常モード=15、広域モード=14）。L14 の 1px は L15 の 2px 相当のため、[FR-008](#fr-008-per-mesh-csv-統合) の stability 判定時に `col_margin_px` を L15 相当へ換算する（[ADR-SRS-004](decisions/ADR-SRS-004-level14-max-pooling-isolated-peaks.md)） |
  ...
  - **広域モード（フェーズ2-2・[FR-014](#fr-014-広域結合解析オーケストレーション) 呼び出し時）の出力動作**:
    - **対象ピーク絞り込み**: 解析範囲内で検出された全ピークではなく、[FR-014](#fr-014-広域結合解析オーケストレーション) から渡された対象ピーク座標リストに含まれるピークの行のみを per-mesh CSV に出力する（merged 統合時のノイズを防ぐため）
...
**説明**:
  ...
  - **未充足ループ（N=4→5→6 のエスカレーション）**:
    1. 抽出した未確定ピーク座標リストと現在の N をフェーズ2-2（[FR-014](#fr-014-広域結合解析オーケストレーション)）に渡して呼び出す。FR-014 が内部で 4 コーナーパターン（NW/NE/SW/SE）の生成・解析・早期終了を行い、広域 per-mesh CSV を出力する
