...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| ピーク候補リスト | 内部トランザクション | ピーク候補レコードのリスト（各レコード: 解析範囲標高グリッド上のピクセル座標・ピーク標高・所属連結成分識別子） | [FR-006](#fr-006-コル検出プロミネンス計算)/[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用)/[FR-016](#fr-016-ピーク域ポリゴン生成) に引き継ぐ。ピクセル座標は当該解析範囲グリッドのスコープでのみ有効（FR-007 で緯度経度へ変換）。連結成分は Union-Find で管理（詳細は HLD 参照） |
