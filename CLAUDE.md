...
`docs/CLAUDE.md` の採番表が定義する各ステージ（URD/SRS/HLD/LLD/COD/UT/IT/ST/OPS）の本体成果物が
確定し、次ステージへ本格着手する前（次ステージの本体ファイルへの最初の編集前）には
`mgmt/phase_gate.md` のゲートレビューを実施する。運用ルール・チェック観点・判定区分・実施記録は
`mgmt/phase_gate.md` を参照。
...
→ 従来どおり `jj1xgo/findsummits` の issue または `todo.md`。起票先リポジトリ名・仕様は
推測せず、不明な場合はユーザーに確認してから起票する。

**例外**: クローズは原則起票側（findsummits）が動作確認後に行うが、調査の結果「仕様どおり・
...
- docs の表記揺れ（記号・用語・書式）を spec-panel レビュー等で同種のものを2回以上修正した場合は、`scripts/lint_docs.py` の検査E規則テーブルへ追加する（1回限りの揺れは追加不要。機械化できる揺れはレビューでなく lint に拾わせる）
