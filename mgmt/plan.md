# 計画: GLOSSARY・SOURCES へのアクティベーションエリア追加

## Context

SRS でアクティベーションエリア（SOTA 公式呼称: アクティベーションゾーン）という新用語が登場した。
...
| 用語 | 正式名称 | 説明 |
|---|---|---|
| アクティベーションエリア（アクティベーションゾーン） | Activation Zone | SOTAルールにおける山頂での運用可能エリア。山頂の最高地点から標高差 25m 以内の連続エリアをさす。このエリア内での無線運用が「山頂からの運用」として認められる。SOTA日本支部では「アクティベーションゾーン」と表記（参照: [SOTA日本支部 FAQ Q12](https://www.kawauchi.homeip.mydns.jp/sotajp/faqs/)）。本プロジェクトではピークと既存SOTAサミットの照合に使用する（FR-016）。 |
...
3. FR-013 GeoJSON フィーチャ構成: アクティベーションエリアポリゴン追加（TBD-03 解消）
4. 新規 FR: アクティベーションエリア計算（findsummits の新機能）
...
SOTAルールの「アクティベーションエリア（サミットから標高差25m以内のエリア）」を活用する。
...
- 出力先: `$DATA_DIR/results/csv/<meshcode>_activation.geojson`（ピークごとのアクティベーションエリア）

**merge.py（Python）が担当:**
- `ref/summitslist.csv` から JA プレフィックスのサミット座標を読み込む
- 各ピークのアクティベーションエリアポリゴンに対して point-in-polygon 判定
- マッチング一意性: プロミネンス≥150m の制約により、1アクティベーションエリア内に複数SOTAサミットは数学的に存在不可

**match_status 判定:**
| match_status | 条件 |
|---|---|
| matched | 検出ピークのアクティベーションエリア内に既存SOTAサミット座標が存在する |
| new | 検出ピークのアクティベーションエリア内に既存SOTAサミット座標が存在しない |
| deleted | いずれの検出ピークのアクティベーションエリアにも含まれない既存SOTAサミット |
...
- ピーク Point クリック → 対応するアクティベーションエリアポリゴンを赤くハイライト
- ハイライト時にアクティベーションエリア内の既存SOTAサミットの有無を視覚確認できる
...
- TBD-03: 解消（GeoJSON フィーチャプロパティ・アクティベーションエリアを本計画で確定）
- ISSUE-006（`tolerance_px = 0`）: 意図的な一時設定。FR-009 のアクティベーションエリア方式への移行により解消
