# 全文書「ユーザー」→「ユーザー」・「キャッシュ」→「ローカルキャッシュ」表記統一
...
- 「ユーザー入力」セクション名・分類名も「ユーザー入力」に変更し、アンカーリンクも同時更新

---

## 作業手順

### Step 1: 「ユーザー」→「ユーザー」全件置換（42件）

全件目視確認済。**すべて機械置換で問題なし**（意図的な「ユーザー」なし、外部引用・固有名詞なし）。
...
**注**: ADR-SRS-016 のファイル名 `ADR-SRS-016-data-classification-external-user-internal.md` は英語スラッグなので変更しない（タイトルの「ユーザー入力」のみ「ユーザー入力」に変更）。

### Step 2: SRS リンクアンカー追従

「ユーザー入力」→「ユーザー入力」によりアンカーが変わる箇所を本文と同時に更新する。

`docs/02_SRS.md`:
- 行74 `[ユーザー入力と内部データ](#7-ユーザー入力と内部データ)` → `[ユーザー入力と内部データ](#7-ユーザー入力と内部データ)`
- 行75 `[7.1 ユーザー入力一覧](#71-ユーザー入力一覧)` → `[7.1 ユーザー入力一覧](#71-ユーザー入力一覧)`
- 行107 `[7.1 ユーザー入力一覧](#71-ユーザー入力一覧)` → 同上

他にアンカー参照箇所が無いことを Step 4 で `grep -rnP "#7-?ユーザー(?!ー)|#71-?ユーザー(?!ー)"` で確認。
...
```bash
# 「ユーザー」（直後ー無し）残存ゼロ確認
grep -rnP "ユーザー(?!ー)" docs/ CLAUDE.md mgmt/plan.md mgmt/lessons.md mgmt/tracker/data/issues.json ref/SOURCES.md

# 旧アンカーリンク残存ゼロ確認
grep -rnP "#7-?ユーザー(?!ー)|#71-?ユーザー(?!ー)" docs/
...
- `docs/02_SRS.md`（ユーザー14 + キャッシュ9）
- `docs/00_GLOSSARY.md`（ユーザー2）
- `docs/decisions/ADR-SRS-014-srs-parameter-description-convention.md`（ユーザー1）
- `docs/decisions/ADR-SRS-015-contour-overlay.md`（キャッシュ1 ← 21行目のみ）
- `docs/decisions/ADR-SRS-016-data-classification-external-user-internal.md`（ユーザー12 + キャッシュ1 ← 32行目）
- `docs/decisions/ADR-SRS-017-internal-transaction-category.md`（ユーザー1）
- `docs/decisions/ADR-URD-005-northern-territories-exclusion.md`（キャッシュ1）
- `CLAUDE.md`（ユーザー4 + キャッシュ4）
- `ref/SOURCES.md`（キャッシュ1）
- `mgmt/plan.md`（ユーザー6 ← 計画記述部分はそのまま、対象は本文の地の文のみ）
- `mgmt/lessons.md`（ユーザー1）
- `mgmt/tracker/data/issues.json`（ユーザー1 + キャッシュ3）
...
- 「ユーザー」一括置換ツールを使う場合は、Python の `re.sub(r"ユーザー(?!ー)", "ユーザー", text)` のように負の先読みで安全に処理する
- 「ユーザー入力」→「ユーザー入力」と「ユーザー」→「ユーザー」の置換順序は逆順（「ユーザー」を先にすると「ユーザー入力」が「ユーザーー入力」になる可能性）。負の先読みを使えば順序問題は発生しないが、念のため `(?!ー)` を使う
