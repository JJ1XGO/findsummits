...
ユーザー選択により **グローバル `CLAUDE.md` (`/home/node/.claude/CLAUDE.md`) の「コア原則」に新項目として明文化** する。全プロジェクト共通で常時コンテキストに載るため、最も強制力が高い。
...
「コア原則」セクションの末尾（既存 `### 4. 学びを活かす` ブロックの直後、26 行目の後）に以下を追加。既存 1〜4 の番号は変更しない。
...
これはプロジェクト固有のトラッカー運用なので、**プロジェクト側の `mgmt/tracker/CLAUDE.md` の運用ルールに移管** する（同ファイルにモデル名記入欄の説明がある場合はそこに、なければ末尾に追記）。

※ 該当ファイルが存在しない場合や記入欄に関する記述がない場合は、`/workspace/CLAUDE.md` の「タスク管理の流れ」セクションに 1 行追記する形で代替する。実装時に該当ファイルを Read して判断する。
...
| `/workspace/mgmt/tracker/CLAUDE.md` または `/workspace/CLAUDE.md` | トラッカー記入時のモデル名表記ルールを追記 | 内（要コミット） |

## 検証

- 次回 Plan Mode を使うセッションで、`ExitPlanMode` 直後に Claude 側から自発的に「Sonnet 切替を促す or Opus 継続を理由付きで申し出る」発話が出るかを観察する
- `/home/node/.claude/CLAUDE.md` を Read し、追加内容が「コア原則」直下に入っていること
- `MEMORY.md` から `feedback_model_switching` 行が消えていること、ファイル本体も消えていること
- プロジェクト CLAUDE.md またはトラッカー CLAUDE.md にバージョン番号なし表記ルールが追記されていること
