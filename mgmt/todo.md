...
- [ ] **(FR-017 spec-panel 指摘①) ADR-URD-005 フォールバック記述を ADR-SRS-018 の決定に合わせて更新**
  - 対象: `docs/decisions/ADR-URD-005-northern-territories-exclusion.md` §Decision フォールバック（行 54-55）
  - 「存在しない場合は除外を行わず、警告ログを出力して処理を続行する」→「FR-001 がエラー終了する（ADR-SRS-018 参照）」に変更

- [ ] **(FR-017 spec-panel 指摘③) FR-017 フォールバックセクションに市区町村 GeoJSON 未生成・タイルリスト未生成の動作を追記**
  - 対象: `docs/20_SRS.md` FR-017 §フォールバック（行 293-294）
  - 追記1: 市区町村 GeoJSON 未生成時は FR-009 が市区町村名を空文字付与して続行する（警告ログ出力）
  - 追記2: 北方領土除外タイルリスト未生成時は FR-001 がエラー終了する（ADR-SRS-018 参照）
...
- [ ] **(FR-017 spec-panel 指摘④) FR-009 説明文（行874）の「市区町村単位の N03 前処理済み地域 GeoJSON」を正しい名称に修正**
  - 対象: `docs/20_SRS.md` FR-009 §説明 行 874
  - 「市区町村単位の N03 前処理済み**地域** GeoJSON」→「N03 前処理済み**市区町村** GeoJSON」に修正
  - FR-009 入力テーブル（行 858）は正しい名称を使っているため、説明文との矛盾解消
