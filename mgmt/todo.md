...
- [ ] **(FR-009 spec-panel 指摘⑥) SRS FR-009 主ピーク特定（行 910）: プロミネンス同値時のタイブレーク規則を追記**
  - 「プロミネンスが最小のピークを主ピークとする」だが同値時の規則が未定義で NFR-003（決定論的出力）との整合が確保されていない
  - 採番順序（行 894-898）と方向を揃えた規則（例: `peak_lat` 降順 → `peak_lon` 昇順）を追記する
  - 対象: `docs/20_SRS.md` FR-009 主ピーク特定（行 910）

- [ ] **(FR-009 spec-panel 指摘⑦) SRS FR-009 既存 SOTA サミット Point プロパティ: unmatched への municipality 不付与の根拠を明示**
  - 行 1030「delete サミットのみ付与（matched / unmatched は空欄）」だが理由が未記述。担当者確認に有用なはずで付与しない根拠を備考に追記する
  - 対象: `docs/20_SRS.md` FR-009 Point: 既存 SOTA サミット プロパティ（行 1030）
