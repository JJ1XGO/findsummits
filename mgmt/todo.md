...
### FR-023 spec-panel 指摘（記述補完・SRS 修正）

- [x] **(FR-023 spec-panel ①) FR-009 不備ゲートに `is_key_col_unresolved` フラグ名を追記**
  - `docs/20_SRS.md` FR-009「不備ゲートと異常終了制御」行943の条件に `is_key_col_unresolved` フラグ名を明記
  - `is_area_incomplete`・`is_unmatched_summit` と同様の記法に統一

- [x] **(FR-023 spec-panel ②) FR-023 手順3 の「N=6 残あり」分岐を追記**
  - `docs/20_SRS.md` FR-023 手順3「残ありかつ N < 6 なら」に「未確定ピークがゼロ**または N = 6**（上限到達）ならフェーズ4 へ」を補記

- [x] **(FR-023 spec-panel ③) FR-023 フェーズ2 単独起動の前提条件を追記**
  - `docs/20_SRS.md` FR-023「各段の独立起動」でフェーズ3/4 の前提条件と並べて、フェーズ2 の前提条件（FR-001 による標高タイル事前取得完了）を追記

- [x] **(FR-023 spec-panel ④) FR-023 手順4 に N インクリメントを追記**
  - `docs/20_SRS.md` FR-023 手順4「完了後に上記 2〜3 を再実施する」→「N をインクリメント（N = N+1）してから上記 2〜3 を再実施する」に修正

- [x] **(FR-023 spec-panel ⑤) FR-023 行379 に FR-009 異常終了後のユーザー対処方針を HLD 委任として付記**
  - `docs/20_SRS.md` FR-023 ループ終了条件「N=6 残あり → フェーズ4 移行、FR-009 異常終了」にHLD 委任として「ユーザー向けエラーメッセージ・対処方法（`merged_summit.xlsx` 確認・未確定ピーク手動解決等）は HLD で定義する」を追記

- [x] **(FR-023 spec-panel ⑥) FR-023 入力テーブル「1次メッシュコードリスト」備考に日本全土リストの直接アクセス有無を明記**
  - `docs/20_SRS.md` FR-023 入力テーブル備考「転送する」→「引数として転送する。省略時は各 FR のデフォルトが使われる（本 FR は日本全土リストを直接読まない）」に修正
...
### FR-007 spec-panel 指摘（記述補完・SRS/ADR 修正）

- [x] **(FR-007 spec-panel ①) FR-006 出力・FR-007 説明にピーク座標の緯度経度変換担当を明示**
  - `docs/20_SRS.md` FR-006 出力備考（行643）: 「ピーク座標はピクセル座標（FR-007 が緯度経度変換する）。コル座標は FR-006 が緯度経度変換済み（`col_lat`/`col_lon`）」と注記を整理
  - `docs/20_SRS.md` FR-007 説明（出力カラム記述の直前）: 「ピーク候補・コル情報リストのピーク座標（ピクセル座標）をズームレベルに基づいて `peak_lat`/`peak_lon`（緯度経度・WGS84）へ変換して出力する。コル座標（`col_lat`/`col_lon`）は FR-006 出力時点で緯度経度変換済み」を追記
  - `docs/decisions/ADR-SRS-020-peak-col-pair-record.md` の Decision フィールド構成: 「コル座標（ピクセル座標）」→「コル座標（`col_lat`/`col_lon`・FR-006 が緯度経度変換済み）」「ピーク座標（ピクセル座標・緯度経度変換は FR-007 が実施）」に更新

- [x] **(FR-007 spec-panel ②) FR-007 説明に3フィルタの適用順序を番号付きで追記**
  - `docs/20_SRS.md` FR-007 説明冒頭（行676の「以下のフィルタを適用する:」の前）に「適用順序: ①プロミネンス一次フィルタ → ②地理的範囲フィルタ → ③対象ピーク絞り込み（広域モードのみ）」の一文を追記

- [x] **(FR-007 spec-panel ③) FR-007 概要文に地理的範囲フィルタへの言及を追記**
  - `docs/20_SRS.md` FR-007 概要文（行656）の末尾に「また地理的範囲フィルタ（解析対象メッシュ外のフリンジピークを除外）を適用する」を追加

### FR-016 spec-panel 指摘（記述補完・SRS 修正）

- [x] **(FR-016 spec-panel ①) FR-007 出力「フィルタ後ピーク候補・コル情報リスト」備考にピーク座標形態を追記、FR-016 共通仕様に緯度経度変換責務を追記**
  - `docs/20_SRS.md` FR-007 出力テーブル「フィルタ後ピーク候補・コル情報リスト」備考: 「ピーク座標はピクセル座標のまま渡す（緯度経度変換は FR-016 が GeoJSON 出力時に担当）」を追記
  - `docs/20_SRS.md` FR-016 共通仕様「計算方法」: ポリゴン頂点座標および `peak_lat`/`peak_lon` のピクセル→WGS84 緯度経度変換責務が本 FR にあることを追記
  - 背景: FR-007 spec-panel ① で「ピーク座標のピクセル→緯度経度変換はFR-007が担当」とFR-006備考に明示したが、FR-007→FR-016への内部トランザクション内のピーク座標形態とFR-016の変換責務への展開が不完全だった

- [x] **(FR-016 spec-panel ②) `area_complete` 判定の false 条件の表現を統一**
  - `docs/20_SRS.md` FR-016 共通仕様（行728相当）: 「解析範囲外で途切れた場合」→「解析対象メッシュ全体の地理的範囲外で途切れた場合」に修正（true 条件と同一概念で統一）

- [x] **(FR-016 spec-panel ③) FR-004 オーケストレーション図の矢印コメントに FR-016 への全入力を明示**
  - `docs/20_SRS.md` FR-004 フロー図（行462相当）: 「↓ FR-007 のフィルタ後ピーク候補・コル情報リストを FR-016 へ渡す」に「※ 解析範囲標高グリッド・解析対象メッシュ範囲情報・解析識別子も同時に引き渡す」を追記
