...
- [ ] **(spec-panel FR-020 ①) FR-020 入力テーブルに外部 I/F（背景タイル・標高タイル・基準点タイル）を追加**
  - FR-019 入力テーブルから「背景タイル（国土地理院標準地図・淡色地図・OSM・OpenTopoMap）| 外部I/F | 必須」「地理院標高タイル（dem5a/5b/5c/10b）・地理院基準点タイル | 外部I/F | 任意」の2行を複写
  - 備考を「公開用 HTML の地図表示時にブラウザから実行時取得（出典・利用形態: [SOURCES.md](../ref/SOURCES.md)）」に調整
  - 対象: `docs/20_SRS.md`（FR-020 入力テーブル 1087行付近）

- [ ] **(spec-panel FR-020 ②) §6.2.8 詳細仕様テーブルに「背景タイル」行を追加**
  - §6.2.6 の「背景タイル | 国土地理院標準地図・国土地理院淡色地図・OSM・OpenTopoMap（切り替え可能。既定: 国土地理院標準地図）」に相当する行を §6.2.8 に追加
  - 対象: `docs/20_SRS.md`（§6.2.8 1551〜1560行付近）
...
- [ ] **(spec-panel FR-020 ④) §6.2.8 にダウンロードファイル名行を追加（HLD委任）**
  - §6.2.9 が `sota_evidence_YYYYMMDD.zip` と明示しているのに対し §6.2.8 はファイル名未定義
  - `ファイル名 | sota_viewer_YYYYMMDD.html（具体的な命名規則は HLD で定義）` 相当の行を追加
  - 対象: `docs/20_SRS.md`（§6.2.8 テーブル 1551行付近）

- [ ] **(spec-panel FR-020 ⑤) §6.2.8「特徴」行に CDN 依存の制約注記を追加**
  - 公開用 HTML は Leaflet（CDN 経由）・各タイルサーバーに依存するため、オフライン環境では地図タイル表示不可である旨を注記
  - 現「特徴 | 自己完結型（GeoJSON 埋め込み・編集 UI なし・XLSX エクスポートなし・localStorage 不使用）」に追記
  - 対象: `docs/20_SRS.md`（§6.2.8 1560行付近）
