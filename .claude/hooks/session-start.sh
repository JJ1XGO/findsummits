...
echo ''
echo '## handover → lessons.md 転記確認（自律実行）'
echo '上記 handover の「学び」「ハマりどころ」「決定事項」セクションを読み、lessons.md に未転記のエッセンスがあれば、このセッションの最初の返答時に lessons.md へ追記すること。'
echo '転記判断基準: 将来のセッションでも繰り返し役立つ教訓・手法・判断パターン。一時的な作業状態や特定タスクの詳細は対象外。'
echo '追記する場合は処方形の記述規約（「〜する」形）に従い make lint を実行する。転記不要と判断した場合は理由を一行で述べること。'
